...
function main(){
 const registry=read(REG),formulaDoc=read(FORM),specs=formulaDoc.markers||{},statusDoc=read(STATUS),dbStatus=statusDoc.statuses||{};
 ...
 for(const m of activeMarkers){const t=tmap.get(m.id);if(!t)continue;if(t.governance_status==='live'){m.provider_status=t.provider_status_override||'live';m.provider_note=t.reason}else if(t.governance_status==='blocked'){m.provider_status=t.provider_status_override||'planned';m.provider_note=t.reason}}
 ...
 fs.writeFileSync(OUT,JSON.stringify({schema_version:3,generated_at:new Date().toISOString(),source_registry:REG,formula_specs:FORM,governance_status_snapshot:STATUS,engine_live_ids:governedLiveIds,governed_live_ids:governedLiveIds,principle:'A derived marker is live only when production Data Center governance records a live or partial executable path. Formula intent alone never promotes a marker. Missing dependencies remain missing rather than receiving synthetic neutral values.',summary,markers:triage},null,2)+'\n');
