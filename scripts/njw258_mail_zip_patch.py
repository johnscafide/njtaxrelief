...
s = replace_once(s, """  var FIELDS = ['PAMS_PIN','COUNTY','MUN_NAME','PROP_LOC','PCLBLOCK','PCLLOT','PCLQCODE',
    ...
    'ZIP5','ST_ADDRESS','CITY_STATE','ADD_LOTS1'].join(',');""", """  // ZIP5, ST_ADDRESS and CITY_STATE on this statewide parcel layer are owner
  ...
  var FIELDS = ['PAMS_PIN','COUNTY','MUN_NAME','PROP_LOC','PCLBLOCK','PCLLOT','PCLQCODE',
    'PROP_CLASS','BLDG_DESC','LAND_DESC','CALC_ACRE','YR_CONSTR','DWELL','COMM_DWELL',
    'LAND_VAL','IMPRVT_VAL','NET_VALUE','LAST_YR_TX','SALE_PRICE','DEED_DATE','DEED_BOOK','DEED_PAGE',
    'ADD_LOTS1'].join(',');""", 'FIELDS')

s = replace_once(s, """  function parcelZip(value) {
...
s = replace_once(s, """  function parcelCandidateMatches(feature, targets) {
...
""", 'parcelCandidateMatches')

s = replace_once(s, """  function parcelAliasIdentityMatches(feature, targets) {
    var a = feature && feature.attributes;
    if (!a) return false;
    var candidate = parcelAddressParts(a.PROP_LOC);
    var candidateZip = String(a.ZIP5 || '').trim();
...
""", 'parcelAliasIdentityMatches')

s = s.replace('sharing the house number + ZIP. Any ambiguity still fails closed.', 'sharing the house number at that location. Any ambiguity still fails closed.')
s = s.replace('land on one\n    // unqualified tax parcel, and that parcel must share house number + ZIP.', 'land on one\n    // unqualified tax parcel, and that parcel must share the searched house number.')
s = s.replace('one unique unqualified parcel with the same house number + ZIP.', 'one unique parcel with the same house number at that location.')

old = """  function parcelByAddressRecord(lat, lon, typed, matched) {
...
"""
new = """  function parcelByAddressRecord(lat, lon, typed, matched) {
    var targets = parcelTargets(typed, matched);
    if (!targets.length) return Promise.resolve(null);
...
"""
s = replace_once(s, old, new, 'parcelByAddressRecord')
s = replace_once(s, 'function recordLookup(p, geo, rate, dy) {', 'function recordLookup(p, geo, rate, dy, propertyZip) {', 'recordLookup signature')
s = replace_once(s, "county: p.COUNTY || '', zip: p.ZIP5 || '', block: p.PCLBLOCK || '',", "county: p.COUNTY || '', zip: propertyZip || '', block: p.PCLBLOCK || '',", 'recordLookup zip')
s = replace_once(s, """    var acres = +p.CALC_ACRE || 0;
...
""", 'render property zip')
s = replace_once(s, 'recordLookup(p, geo, rate, dy);', 'recordLookup(p, geo, rate, dy, propertyZip);', 'recordLookup call')
owner_line = "            frow('Owner mailing', absentee ? [p.ST_ADDRESS, p.CITY_STATE].filter(Boolean).join(', ') : 'Same as property') +\n"
if owner_line not in s:
    raise SystemExit('missing owner mailing line')
s = s.replace(owner_line, '', 1)
s = replace_once(s, "outFields: 'PAMS_PIN,PROP_LOC,MUN_NAME,COUNTY,ZIP5,PCLBLOCK,PCLLOT,NET_VALUE,LAST_YR_TX,' +", "outFields: 'PAMS_PIN,PROP_LOC,MUN_NAME,COUNTY,PCLBLOCK,PCLLOT,NET_VALUE,LAST_YR_TX,' +", 'hood fields')
s = replace_once(s, "county: a.COUNTY || '', zip: a.ZIP5 || '',", "county: a.COUNTY || '', zip: '',", 'hood zip')
p.write_text(s)

a = Path('property/js/nj-address-autocomplete.js')
t = a.read_text()
t = replace_once(t, 'LAST_YR_TX,ZIP5,YR_CONSTR', 'LAST_YR_TX,YR_CONSTR', 'autocomplete fields')
anchor = "function streetNumber(v){var m=String(v||'').trim().match(/^(\\d+[A-Z-]?)/i);return m?m[1].toUpperCase():'';}"
helper = anchor + "\n  function zipFromAddress(v){var m=String(v||'').match(/\\b(\\d{5})(?:-\\d{4})?\\b/);return m?m[1]:'';}"
t = replace_once(t, anchor, helper, 'autocomplete ZIP helper')
t = replace_once(t, "pams_pin:a.PAMS_PIN||'',address:a.PROP_LOC||address,town:a.MUN_NAME||'',county:a.COUNTY||'',zip:a.ZIP5||'',", "pams_pin:a.PAMS_PIN||'',address:a.PROP_LOC||address,town:a.MUN_NAME||'',county:a.COUNTY||'',zip:zipFromAddress(address),", 'autocomplete row zip')
a.write_text(t)

test = Path('property/tests/property-address-alias-contract.mjs')
u = test.read_text()
u = replace_once(u, "assert.match(lookup, /target\\.zip === candidateZip/, 'alias candidate must keep the same ZIP');", "assert.doesNotMatch(lookup, /candidateZip = String\\(a\\.ZIP5/, 'owner mailing ZIP must never participate in parcel alias identity');", 'alias test ZIP assertion')
test.write_text(u)

Path('property/tests/property-mailing-zip-contract.mjs').write_text("""import assert from 'node:assert/strict';
