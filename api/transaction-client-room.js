...
try{
 ...
 const txs=await rest('transaction_workspaces?select=id,address,city,state,postal_code,municipality,county,status,contract_date,closing_date,client_label,updated_at&id=eq.'+encodeURIComponent(room.transaction_id)+'&limit=1',c),tx=txs[0];
