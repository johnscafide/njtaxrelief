...
async function sha256(value) { return require('node:crypto').createHash('sha256').update(value).digest('hex'); }
...
async function jsonFetch(url, options = {}) {
  ...
  if (!response.ok) {
    const message = data && typeof data === 'object' ? (data.message || data.error_description || data.error || data.hint) : data;
...
async function upload(path, bytes, mime) {
  const response = await fetch(`${SUPABASE_URL}/storage/v1/object/${BUCKET}/${pathEncode(path)}`, {
    method: 'POST',
    headers: adminHeaders({ 'Content-Type': mime, 'x-upsert': 'false', 'Cache-Control': '3600' }),
    body: bytes
  });
...
function stylePrompt(campaign, briefRow, variant, preset) {
  ...
  return [
    ...
    'For residential imagery, use a non-identifiable New Jersey / Northeast suburban home or streetscape atmosphere rather than a specific real property. For analytical concepts, use abstract parcel/map/property geometry without literal data values.',
...
async function gatewayImage(prompt, userId) {
  if (!GATEWAY_TOKEN) throw new Error('Studio image provider is not configured');
  const model = process.env.WATCHDOG_IMAGE_MODEL || 'openai/gpt-image-2';
  const response = await fetch('https://ai-gateway.vercel.sh/v1/images/generations', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${GATEWAY_TOKEN}`,
      'Content-Type': 'application/json',
      'ai-reporting-user': userId,
      'ai-reporting-tags': 'feature:watchdog-studio-visual,product:marketing-studio'
    },
    body: JSON.stringify({ model, prompt, n: 1, size: '1536x1024', quality: 'medium', response_format: 'b64_json' })
  });
  const text = await response.text();
  let data = {};
  try { data = JSON.parse(text); } catch { data = {}; }
  if (!response.ok) throw new Error(clean(data?.error?.message || data?.error || `Image generation failed (${response.status})`, 500));
  const encoded = data?.data?.[0]?.b64_json;
  if (!encoded) throw new Error('Image provider returned no generated image');
  return { bytes: Buffer.from(encoded, 'base64'), model, response: data };
}
async function logEvent(userId, campaignId, eventType, payload) {
  try { await adminInsert('marketing_events', { user_id: userId, campaign_id: campaignId, event_type: eventType, source: 'watchdog_intelligence', payload }); } catch (_) { /* event logging must not break the workflow */ }
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'status') return res.status(200).json({
      ...
      image_model: process.env.WATCHDOG_IMAGE_MODEL || 'openai/gpt-image-2',
      bucket: BUCKET,
      production_boundary: 'preview_only_until_pcm_mapping_and_proof'
    });

    if (action === 'styles') {
      const q = new URLSearchParams({ select: 'preset_key,label,description,minimum_tier,art_direction,sort_order', active: 'eq.true', order: 'sort_order.asc' });
      const styles = await adminSelect('marketing_visual_style_presets', q);
      return res.status(200).json({ styles: Array.isArray(styles) ? styles : [] });
    }
    ...
    if (action === 'list') {
      const q = new URLSearchParams({
        ...
        user_id: `eq.${user.id}`, campaign_id: `eq.${campaignId}`, order: 'created_at.desc', limit: '100'
      });
      const rows = await adminSelect('marketing_intelligence_visual_assets', q);
      const assets = await Promise.all((Array.isArray(rows) ? rows : []).map(async x => ({ ...x, signed_url: await signedUrl(x.storage_path) })));
    ...
    if (action === 'select') {
      ...
      const aq = new URLSearchParams({ select: '*', id: `eq.${assetId}`, user_id: `eq.${user.id}`, campaign_id: `eq.${campaignId}`, limit: '1' });
      const assets = await adminSelect('marketing_intelligence_visual_assets', aq);
      const asset = Array.isArray(assets) ? assets[0] : null;
      if (!asset || asset.status === 'failed') return res.status(404).json({ error: 'Visual asset not found.' });

      await adminPatch('marketing_intelligence_visual_assets', `user_id=eq.${encodeURIComponent(user.id)}&campaign_id=eq.${encodeURIComponent(campaignId)}&status=eq.selected&id=neq.${encodeURIComponent(assetId)}`, { status: 'superseded', updated_at: new Date().toISOString() });
      const selectedRows = await adminPatch('marketing_intelligence_visual_assets', `id=eq.${encodeURIComponent(assetId)}&user_id=eq.${encodeURIComponent(user.id)}`, { status: 'selected', production_status: 'preview_only', selected_at: new Date().toISOString(), updated_at: new Date().toISOString() }, true);
      const selected = Array.isArray(selectedRows) ? selectedRows[0] : asset;
      const settings = { ...safe(campaign.settings), direct_mail: { ...safe(campaign.settings?.direct_mail), studio_visual_asset_id: assetId, studio_visual_status: 'preview_only', studio_visual_style: selected.style_preset_key } };
      await adminPatch('marketing_campaigns', `id=eq.${encodeURIComponent(campaignId)}&user_id=eq.${encodeURIComponent(user.id)}`, { settings, updated_at: new Date().toISOString() });

      const cq = new URLSearchParams({ select: 'id,content', user_id: `eq.${user.id}`, campaign_id: `eq.${campaignId}`, channel: 'eq.direct_mail', status: 'eq.draft', order: 'created_at.desc', limit: '1' });
      const creativeRows = await adminSelect('marketing_creatives', cq);
      const creative = Array.isArray(creativeRows) ? creativeRows[0] : null;
      if (creative) {
        const content = { ...safe(creative.content), visual_asset_id: assetId, visual_storage_path: selected.storage_path, visual_asset_status: 'preview_only', visual_style_preset: selected.style_preset_key };
        await adminPatch('marketing_creatives', `id=eq.${encodeURIComponent(creative.id)}&user_id=eq.${encodeURIComponent(user.id)}`, { visual_asset_id: assetId, content, updated_at: new Date().toISOString() });
        await adminPatch('marketing_intelligence_visual_assets', `id=eq.${encodeURIComponent(assetId)}&user_id=eq.${encodeURIComponent(user.id)}`, { creative_id: creative.id, updated_at: new Date().toISOString() });
      }
      await logEvent(user.id, campaignId, 'creative.studio_visual_selected', { asset_id: assetId, brief_id: selected.brief_id, variant_index: selected.variant_index, style_preset_key: selected.style_preset_key, production_status: 'preview_only' });
      return res.status(200).json({ asset: { ...selected, signed_url: await signedUrl(selected.storage_path) }, production_boundary: 'PCM proof still required.' });
    }

    if (action !== 'generate') return res.status(400).json({ error: 'Unsupported action.' });
    if (!GATEWAY_TOKEN) return res.status(503).json({ error: 'Studio image provider is not configured.' });
    ...
    const pendingRows = await adminInsert('marketing_intelligence_visual_assets', {
      user_id: user.id, campaign_id: campaignId, brief_id: briefRow.id, variant_index: variantIndex,
      style_preset_key: styleKey, asset_kind: 'generated_preview', status: 'generating', production_status: 'preview_only',
      provider: 'vercel_ai_gateway', model: process.env.WATCHDOG_IMAGE_MODEL || 'openai/gpt-image-2', prompt, prompt_hash: promptHash,
    ...
    try {
      const generated = await gatewayImage(prompt, user.id);
      ...
      const doneRows = await adminPatch('marketing_intelligence_visual_assets', `id=eq.${encodeURIComponent(pending.id)}&user_id=eq.${encodeURIComponent(user.id)}`, {
        status: 'generated', storage_path: path, mime_type: 'image/png', width: 1536, height: 1024, bytes: generated.bytes.length,
        metadata: { engine_version: ENGINE_VERSION, creative_tier: 'studio', mechanical_spec_owner: 'pcm', gateway_model: generated.model, usage: safe(generated.response?.usage, 5000), size: '1536x1024', quality: 'medium' },
        updated_at: new Date().toISOString()
      }, true);
      const done = Array.isArray(doneRows) ? doneRows[0] : null;
      await logEvent(user.id, campaignId, 'creative.studio_visual_generated', { asset_id: pending.id, brief_id: briefRow.id, variant_index: variantIndex, style_preset_key: styleKey, provider: 'vercel_ai_gateway', model: generated.model, production_status: 'preview_only' });
      return res.status(200).json({ ok: true, asset: { ...done, signed_url: await signedUrl(path) }, production_boundary: 'Watchdog preview only. PCM mapping and provider proof are still required before production.' });
    } catch (error) {
      await adminPatch('marketing_intelligence_visual_assets', `id=eq.${encodeURIComponent(pending.id)}&user_id=eq.${encodeURIComponent(user.id)}`, { status: 'failed', metadata: { engine_version: ENGINE_VERSION, error: clean(error?.message || error, 500) }, updated_at: new Date().toISOString() });
      throw error;
    }
  } catch (error) {
    console.error('marketing-studio-visual failure', error && error.message ? error.message : 'unknown');
    const status = Number(error?.status || 0);
    return res.status(status >= 400 && status < 500 ? status : 502).json({ error: clean(error?.message || 'Studio Visual Engine is temporarily unavailable.', 500) });
  }
};
