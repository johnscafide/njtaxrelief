...
if(req.method==='GET'){
 try{
   const rows=await rest('agent_open_houses?select=id,user_id,title,address,city,municipality,county,postal_code,event_at,status,pams_pin&event_code=eq.'+encodeURIComponent(code)+'&status=in.(scheduled,live)&limit=1',access.c),event=rows[0];
   if(!event)return res.status(404).json({error:'This open house is unavailable.'});
   const profs=await rest('profiles?select=id,account_role,pro_agent,photo_url,avatar_url&id=eq.'+encodeURIComponent(event.user_id)+'&limit=1',access.c),p=profs[0]||{},ents=await rest('account_entitlements?select=subscription_status,billing_tier,plan_tier&user_id=eq.'+encodeURIComponent(event.user_id)+'&limit=1',access.c),ent=ents[0]||{},tier=String(ent.billing_tier||ent.plan_tier||'').toLowerCase().replace('pro+','pro_plus'),active=p.account_role==='developer'||(['active','trialing','past_due','cancel_scheduled'].includes(String(ent.subscription_status||'').toLowerCase())&&['agent','pro','pro_plus','teams'].includes(tier));if(!active)return res.status(404).json({error:'This open house is unavailable.'});const brand=p.pro_agent&&typeof p.pro_agent==='object'?p.pro_agent:{};
