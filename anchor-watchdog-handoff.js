(function(){
  ...
  function resultVisible(){
    ...
    return !!(step&&step.classList.contains('active')&&host&&String(host.textContent||'').trim());
  }
  ...
  function stage(params){
    if(staging||attempted||finished)return;
    ...
    attempted=true;staging=true;overlay();
    var answers=selectedAnswers();
    fetch(URL,{method:'POST',headers:{'Content-Type':'application/json','apikey':KEY,'Authorization':'Bearer '+KEY},body:JSON.stringify({action:'stage',email:email,code:code,result:{name:name,address:address,answers:answers,intent_score:params&&params.intent_score}})})
      ...
      .then(function(body){
        ...
        setTimeout(function(){location.replace('https://www.watchdogindex.com/#anchor-result='+token);},250);
      })
  ...
  function maybeStage(){
    if(attempted||finished||!resultVisible())return;
  ...
  function bindResultFallback(){
    ...
    setTimeout(maybeStage,350);
    setTimeout(maybeStage,1500);
  }
