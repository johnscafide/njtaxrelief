...
def main():
    ...
    for ws in wb.worksheets:
        # scan all rows in dictionary/introduction; first 12 rows in data sheets are sufficient for headers
    ...
    payload={'schema_version':1,'generated_at':datetime.now(timezone.utc).isoformat(),'source_url':SOURCE_URL,'source_bytes':len(r.content),'hits':hits,'hit_count':len(hits)}
