...
const enhancementsJs = read('property/js/anchor-application-2025-enhancements.js');
const enhancementsPartial = read('property/partials/anchor-application-2025-enhancements.html');
const growthJs = read('property/js/anchor-application-2025-growth.js');
const growthPartial = read('property/partials/anchor-application-2025-growth.html');
const supportCheckout = read('supabase/functions/create-support-checkout/index.ts');
...
assert.match(enhancementsJs, /\?address=/);
assert.match(growthPartial, />Save<\/button>/);
assert.match(growthPartial, /Save &amp; continue in Watchdog/);
assert.match(growthPartial, /Continue your Watchdog setup/);
assert.match(growthJs, /create-support-checkout/);
assert.match(supportCheckout, /watchdog_optional_support/);
assert.match(supportCheckout, /access_dependency:'none'/);
