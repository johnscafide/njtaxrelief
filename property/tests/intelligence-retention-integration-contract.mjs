...
const paths={
  ...
  universal:'property/js/watchdog-universal-menu.js',
...
const today=read(paths.today),css=read(paths.css),universal=read(paths.universal),dashboard=read(paths.dashboard),home=read(paths.home),appShell=read(paths.appShell),daily=read(paths.daily),partial=read(paths.partial),analytics=read(paths.analytics);
const expect=(ok,msg)=>{if(!ok)throw new Error(msg)};
new vm.Script(today,{filename:paths.today});
new vm.Script(universal,{filename:paths.universal});
...
expect(css.includes('.wd-today-badge'),'Legacy Today cue must remain a compact badge, not a large Intelligence panel.');
...
// Modern app navigation is owned by the universal menu. The legacy Today helper
// intentionally does not mutate universal-menu nodes, so the canonical Pro+ route
// must live directly in the universal registry rather than being injected later.
expect(universal.includes("key:'today'") && universal.includes("href:route('/intelligence/daily')") && universal.includes("label:'Today'"),'Universal navigation must expose the Today return route.');
expect(universal.includes("if(state.ready && can('pro_plus')) out.push({key:'today'"),'Universal Today route must remain Pro+ gated.');
expect(universal.includes("item.key === 'today'") && universal.includes('/intelligence\\/daily'),'Universal Today route must preserve its active-page state.');
expect(dashboard.includes('/property/js/product-analytics.js'),'Dashboard must participate in first-party product analytics.');
expect(dashboard.includes('/property/js/brand-consistency-runtime.js'),'Dashboard must remain connected to the universal navigation runtime.');
expect(home.includes('/property/js/watchdog-today-nav.js'),'Property Home must keep the legacy Today badge helper for compatible side-primary surfaces.');
expect(appShell.includes('/property/js/watchdog-today-nav.js'),'Modern secondary shell must keep the legacy Today badge helper where compatible side-primary navigation exists.');
