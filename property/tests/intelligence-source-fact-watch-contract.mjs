...
for(const requiredEvent of ['assessment_change','tax_change','deed_change'])must(fn.includes(`event:"${requiredEvent}"`),`Watcher must preserve Property Change v4 evidence diversity via ${requiredEvent}.`);
...
console.log('Intelligence source-fact watch contract passed: env-safe disablement, Vault token auth, paid-plan gate, current official NJ service resolution, baseline-first/idempotent semantics, three-type Property Change evidence taxonomy, service-only research candidates, no notifications/findings, six-hour cadence, no ?v=.');
