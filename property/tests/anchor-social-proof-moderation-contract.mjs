...
const anchorPartial = read('property/partials/anchor-home-funnel.html');
...
must(anchorPartial.includes('href="https://propertytaxreliefstatus.nj.gov/"'), 'Status card must link to the official NJ Property Tax Relief Status Checker.');
must(anchorPartial.includes('referrerpolicy="no-referrer"'), 'Official status handoff must suppress the Watchdog referrer.');
must(anchorPartial.includes('rel="noopener noreferrer"'), 'Official status handoff must isolate the new tab.');
must(!anchorPartial.includes('propertytaxreliefstatus.nj.gov/?') && !anchorPartial.includes('propertytaxreliefstatus.nj.gov?'), 'SSN/ITIN or ZIP must never be appended to the official status URL.');
must(anchorPartial.includes('data-anchor-review-prev aria-label="Previous review"') && anchorPartial.includes('data-anchor-review-next aria-label="Next review"'), 'Review carousel must have accessible previous/next controls.');
must(rating.includes('controls.hidden=reviewItems.length<2'), 'Review arrows must be hidden until multiple approved reviews exist.');
...
must(anchorPartial.includes('data-anchor-review-panel hidden'), 'Written review panel must begin hidden until approved review data exists.');
must(anchorPartial.includes('Track your filed application'), 'Official status card must remain available independently of written reviews.');
...
must(rating.includes('data-anchor-testimonials'), 'Approved testimonials and status handoff must use the ANCHOR post-file host.');
