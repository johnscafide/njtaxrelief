...
const files={
  ...
  dashboardPage:'property/dashboard/index.html',
...
const dashboardPage=fs.readFileSync(files.dashboardPage,'utf8');
...
expect(!dashboardPage.includes('/property/js/watchdog-dashboard-v2-intelligence.js'),'2027 Dashboard must not restore the retired Dashboard-v2 Intelligence density bridge.');
expect(dashboardPage.includes('/property/js/dashboard/wd-intel.js'),'2027 Dashboard must keep its page-native Intelligence drawer rather than depending on the retired density bridge.');
expect(home.includes('/property/js/watchdog-intelligence-density.js'),'Property Home must load the density runtime.');
expect(!home.includes('/property/js/watchdog-intelligence-density.js?v='),'Property Home density boundary must remain on its canonical unversioned URL.');
...
for(const [name,content] of Object.entries({runtime,css,homeCore}))expect(!content.includes('?v='),`${name} must not introduce ?v= asset URLs.`);
console.log('Intelligence density contract passed: compact supporting surfaces, <=3 reasons, one next action, progressive disclosure, immersive-route exemption, current Dashboard drawer, stable Home Intelligence boundary, Analyst naming, idempotency and mobile.');
