...
for(const [name,content] of Object.entries({runtime,css,dashboard,home}))expect(!content.includes('?v='),`${name} must not introduce ?v= asset URLs.`);
console.log('Intelligence density contract passed: compact supporting surfaces, <=3 reasons, one next action, progressive disclosure, immersive-route exemption, idempotency, mobile, no ?v=.');
