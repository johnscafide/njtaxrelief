...
must(shellCss.includes('.wdd-work>.wdd-main-column') && shellCss.includes('#wdd-adaptive{') && shellCss.includes('.wdd-work>.wdd-rail') && shellCss.includes('grid-row:1') && visualCore.includes("q('#wdd-positions',work).appendChild(host)") && page.includes('wdd-main-column') && page.includes('wdd-newsslot'), 'Desktop workspace must keep the portfolio, summary and news in one flowing main column beside the single-row right rail.');
