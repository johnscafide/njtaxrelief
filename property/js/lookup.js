...
(function () {
  ...
  function lookupAtPoint(lat, lon, label) {
    ...
    el('pl-inline').innerHTML =
      ...
      '<div class="pl-state-title">Searching state records</div>' +
      '<div class="pl-state-sub">Finding the parcel at your location.</div></div>';
    ...
    Promise.all([
      ...
      .then(function (f) {
        ...
        render(f, { lat: lat, lon: lon, matched: label }, label);
      })
      .catch(function (e) {
  ...
  window.plLookup = function () {
    ...
    // If the inline status strip has been removed from the page, show progress
    // in the search button instead so there is still visible feedback.
    ...
    el('pl-inline').innerHTML =
      '<div class="pl-state"><div class="pl-spin"></div>' +
      '<div class="pl-state-title">Searching state records</div>' +
      '<div class="pl-state-sub">Matching ' + esc(addr) + ' to its parcel.</div></div>';
    if (typeof gtag === 'function') gtag('event', 'property_lookup');

    // The four data files are optional. If any of them is slow or missing the
    // lookup must still run, so cap the whole preload and move on regardless.
    ...
    capped
      ...
      .then(function (res) {
        ...
        el('pl-inline').innerHTML = readyState();
        // Render separately so a bug in the panel cannot masquerade as
        // "address not found". Those are completely different problems and
        // telling someone their address does not exist when the real fault is
        // ours is the worst possible answer.
        try {
          render(res.f, res.g, addr);
        } catch (err) {
      ...
      .catch(function (e) {
        ...
        if (m === 'nogeo') {
          ...
          msg = 'Try adding the town and zip code, or drop the unit number. ' +
                'New construction and some condo units are not in the state file yet.';
        } else if (m === 'noparcel') {
          title = 'No parcel record matched';
          msg = 'We found the location but no parcel record there. That usually means a condo unit, ' +
                'a brand new build, or a property billed together with other lots.';
          ...
          extra = '<div class="pl-src" style="margin-top:14px;font-size:12px;">Technical detail: ' +
                  esc(m.slice(7)) + '</div>';
        } else if (m === 'timeout') {
          title = 'The state records service timed out';
          msg = 'New Jersey\u2019s property server accepted the request and never answered. ' +
                'That happens now and then and usually clears within a few minutes. Try again shortly.';
        } else {
          title = 'That lookup did not go through';
          msg = 'The state property service did not answer. It goes down occasionally. ' +
        ...
        el('pl-inline').innerHTML =
          ...
          '<div class="pl-src" style="margin-top:20px;">If you know your block and lot, your municipal tax assessor can pull the exact record. <a href="/index.html#contact">Send me the address</a> and I will look it up by hand.</div>' +
          extra;
      })
      .then(function () {
        btn.disabled = false;
  ...
  function recordLookup(p, geo, rate, dy) {
    try {
      ...
      if (!LEDGER_URL || !LEDGER_KEY) return;
      xfetch(LEDGER_URL.replace(/\/+$/, '') + '/rest/v1/rpc/record_lookup', 8000, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', apikey: LEDGER_KEY,
...
(peerGap > 0 ? '+' : '') + money(peerGap) + ' (' + (peerPct > 0 ? '+' : '') + peerPct.toFixed(1) + '%)</b></div>' +
  ...
  // ══════════════════════════════════════════════
  function render(feat, geo, typed) {
    ...
    el('plm-main').innerHTML =
      ...
      '<div class="plm-sec">' + gate('tradeup',
    ...
    // every single lookup. These are free public servers and we should not
    // hammer them for data we throw away.
    // liveRatio used to run here, querying up to 2,000 town records to work out
    // which deeds were real sales. SR1A already carries the state's own verdict
    // on every deed, so that whole query is gone.
    ...
    Promise.all([
      ...
      if (own) {
        if (own.sf) {
          ...
          if (lq) lq.innerHTML = '<i class="fas fa-house"></i><div><b>' + own.sf.toLocaleString() +
            '</b><span>Living sq ft</span></div>';
        }
      ...
      var val = watchdogValuation(subject, comps, new Date().getFullYear(),
                                  current.town, current.county, sr1a || offR,
                                  sr1a && sr1a.drift != null ? sr1a.drift : null);
