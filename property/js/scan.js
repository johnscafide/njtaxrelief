...
(function () {
  ...
  window.scCounty = function (code) {
    ...
    town.innerHTML = '<option value="">Choose a municipality</option>' + towns.map(function (item) {
      return '<option value="' + esc(item.district) + '">' + esc(item.name) + '  (' + Number(item.verified_sales || 0).toLocaleString() + ' verified sales)</option>';
    }).join('');
    town.disabled = false;
  };
  ...
  function reasonContent(hit) {
    ...
    return '<strong>Why this scored ' + Number(hit.opportunity && hit.opportunity.score || 0) + '</strong>' +
      '<p>The current assessment is <b>' + money(hit.over) + '</b> above the Chapter 123 screening limit derived from this parcel\'s own NJ-verified ' + hit.y + ' sale.</p>' +
      '<dl><div><dt>Verified sale</dt><dd>' + money(hit.p) + '</dd></div><div><dt>Time-adjusted market</dt><dd>' + money(hit.market) + '</dd></div><div><dt>Supported assessment</dt><dd>' + money(hit.fair) + '</dd></div><div><dt>Ch. 123 limit</dt><dd>' + money(hit.limit) + '</dd></div></dl>' +
      ...
      '<p class="sc-sources">The screening result is calculated server-side from governed Watchdog source inputs. Sources include NJ Division of Taxation equalization and verified-sale data.</p>';
  }
  function reasonTip() {
  ...
  window.scQuickAdd = function (index, event) {
    ...
    sb.rpc('save_property', { p: {
      ...
      effective_rate: +(lastRun.rate * 100).toFixed(3), watchdog_value: Math.round(hit.market)
  ...
  window.scRun = function () {
    ...
    scanner({ action: 'scan', district: district, max_years: maxYears, min_saving: minSaving }).then(function (data) {
      if (data.result === 'insufficient_sales') {
        el('sc-out').innerHTML = '<div class="sc-none"><b>Not enough verified sales</b><p>Only ' + Number(data.pool || 0) + ' arm\u2019s length sales are on file for this town inside that window. Widen the window or pick a larger municipality.</p></div>';
        return;
      }
  ...
  function paint() {
    ...
    el('sc-out').innerHTML = '<div class="sc-res">' +
      '<div class="sc-sum"><div><b>' + run.hits.length.toLocaleString() + '</b><span>properties above the screen</span></div><div><b>' + Math.round(run.hits.length / Math.max(1, run.pool) * 100) + '%</b><span>of the ' + run.pool + ' sales screened</span></div><div><b>' + priority.toLocaleString() + '</b><span>high-priority opportunity files</span></div><div><b>' + money(totalSaving) + '</b><span>annual tax at stake, combined</span></div></div>' +
      '<p class="sc-lede">In <b>' + esc(run.name) + '</b>, <b>' + run.hits.length + '</b> of the <b>' + run.pool + '</b> homes that sold between <b>' + run.from + '</b> and <b>' + run.to + '</b> now carry an assessment above the Chapter 123 screen measured against their own verified sale. ' + coefficientCopy + appealCopy + '</p>' +
      ...
      '<div class="sc-tablewrap"><table class="sc-t"><thead><tr>' + sortHead('opportunity','Opportunity') + sortHead('address','Address') + sortHead('block','Blk/Lot') + sortHead('year','Sold') + sortHead('sale','Sale price',true) + sortHead('assessed','Assessed',true) + sortHead('limit','Ch123 limit',true) + sortHead('over','Over by',true) + sortHead('saving','Saving/yr',true) + sortHead('evidence','Evidence') + '</tr></thead><tbody>' +
      ...
      '<div class="sc-method"><h4>How this was calculated, and what it is not</h4><p>The Pro+ ranking is computed <b>server-side</b>. The browser sends the municipality and bounded screening options; it does not download the county sales file or calculate the opportunity score.</p><p>Each row uses the subject property\u2019s own verified sale. The sale is carried forward at <b>' + (run.drift * 100).toFixed(1) + '% a year</b> from the governed municipality context, then multiplied by the municipality ratio of <b>' + (run.ratio * 100).toFixed(1) + '%</b>. The Chapter 123 cushion is applied by the server-side formula. Annual saving uses an effective rate of ' + (run.rate * 100).toFixed(1) + '%.</p><p class="sc-warn"><b>This is a screening list, not a filing list.</b> Public records cannot see property condition, renovations or other facts that can explain an assessment gap. Every row needs human review before action. It is not a consumer report and may not be used for tenant screening, employment or credit decisions.</p><p class="sc-sources">Formula: ' + esc(run.formulaVersion || 'governed server version') + '.</p></div></div>';
  ...
  function rowsOut() {
    return sortedHits().map(function (hit) {
      return {
        Address: hit.a, Block: hit.b, Lot: hit.l, Town: lastRun.name,
        Sale_Year: hit.y, Sale_Price: hit.p, Assessed: hit.av,
        Living_SqFt: hit.sf || '', Year_Built: hit.yb || '',
        Market_Today: Math.round(hit.market), Supported_Assessment: Math.round(hit.fair),
        Ch123_Limit: Math.round(hit.limit), Over_Limit_By: Math.round(hit.over),
        Est_Annual_Saving: Math.round(hit.saving),
        Assessed_Pct_Of_Sale: (Number(hit.implied || 0) * 100).toFixed(1),
