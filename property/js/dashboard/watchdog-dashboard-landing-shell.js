...
(function () {
  ...
  function ago(value) {
    ...
    var days = Math.max(0, Math.round(diff / 86400000));
  ...
  function bindShell() {
    ...
    document.addEventListener('click', function (event) {
      if (window.innerWidth <= 900 && document.body.classList.contains('wdm-menu-open') &&
          !event.target.closest('#wdm-sidebar') && !event.target.closest('#wdm-menu-toggle')) {
        document.body.classList.remove('wdm-menu-open');
      }
      if (menu && !menu.hidden && !event.target.closest('#wdm-account') && !event.target.closest('#wdm-account-menu')) menu.hidden = true;
    });
    ...
    document.querySelectorAll('[data-wdm-scroll]').forEach(function (node) {
  ...
  function scoreValue(score) {
    ...
    if (value == null) value = num(score.watchdog_score);
    return value;
  }
  ...
  function chartMarkup(WD, property, currentScore) {
    ...
    if (property && property.pams_pin) rows = rows.filter(function (r) { return r.pams_pin === property.pams_pin; });
    rows = rows.map(function (r) {
      var score = num(r.score);
      if (score == null) score = num(r.watchdog_score);
      return { score:score, at:r.observed_at || r.computed_at || r.created_at || null };
    }).filter(function (r) { return r.score != null; }).sort(function (a,b) { return new Date(a.at || 0) - new Date(b.at || 0); });
    ...
    if (!rows.length) {
      return '<div class="wdm-change-empty">Watchdog Score history will appear as observations accumulate for this property.</div>';
    }
    var width = 320, height = 108, padX = 8, padY = 10;
    var pts = rows.map(function (r, i) {
      var x = padX + (rows.length === 1 ? 0 : i * ((width - padX * 2) / (rows.length - 1)));
      var y = padY + (100 - Math.max(0, Math.min(100, r.score))) * ((height - padY * 2) / 100);
      return {x:x,y:y,score:r.score};
    });
    var poly = pts.map(function (p) { return p.x.toFixed(1)+','+p.y.toFixed(1); }).join(' ');
    var fill = 'M '+pts[0].x.toFixed(1)+' '+(height-padY)+' L '+pts.map(function(p){return p.x.toFixed(1)+' '+p.y.toFixed(1);}).join(' L ')+' L '+pts[pts.length-1].x.toFixed(1)+' '+(height-padY)+' Z';
    var dots = pts.map(function (p) { return '<circle class="wdm-chart-dot" cx="'+p.x.toFixed(1)+'" cy="'+p.y.toFixed(1)+'" r="3"/>'; }).join('');
    return '<div class="wdm-score-chart"><svg viewBox="0 0 '+width+' '+height+'" role="img" aria-label="Watchdog Score history">'+
      '<defs><linearGradient id="wdm-chart-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#80caff" stop-opacity=".5"/><stop offset="1" stop-color="#d9f1ff" stop-opacity=".12"/></linearGradient></defs>'+
      '<path class="wdm-gridline" d="M0 22H320M0 54H320M0 86H320"/><path class="wdm-chart-fill" d="'+fill+'"/><polyline class="wdm-chart-line" points="'+poly+'"/>'+dots+'</svg>'+
      '<span class="wdm-chart-note">Historical Watchdog Score observations for the selected property</span></div>';
  }
  ...
  function changesMarkup(WD, property) {
    ...
    changes.sort(function (a,b) { return new Date(b.occurred_at || b.created_at || 0) - new Date(a.occurred_at || a.created_at || 0); });
    changes = changes.slice(0,3);
    if (!changes.length) return '<div class="wdm-change-empty">No recent verified changes are available for this property yet. Watchdog will surface them here as underlying records update.</div>';
    return '<div class="wdm-change-list">'+changes.map(function (event) {
      ...
      return '<div class="wdm-change"><span class="wdm-change-icon"><i class="fas '+eventIcon(event)+'" aria-hidden="true"></i></span><div><b>'+esc(label)+'</b><small>'+esc(ago(event.occurred_at || event.created_at))+'</small></div></div>';
    }).join('')+'</div>';
  }
  ...
  function renderOverview(force) {
    ...
    var stats = typeof WD.stats === 'function' ? WD.stats() : {};
    var score = scoreObject(WD, property);
    var scoreNum = scoreValue(score);
    var signature = [property && property.pams_pin, property && property.assessed, property && property.watchdog_value, property && property.last_year_tax, scoreNum, WD.S.changes && WD.S.changes.length, WD.S.scoreHistory && WD.S.scoreHistory.length, firstName(), planLabel()].join('|');
    ...
    if (!property) {
      overview.innerHTML = '<div class="wdm-property"><img class="wdm-property-photo" src="/agent/assets/property-house.webp" alt="" width="148" height="94"><div class="wdm-property-info"><h1>Your Watchdog dashboard</h1><p>Save a New Jersey property to build this intelligence view.</p><span class="wdm-property-type">Property intelligence</span><ul class="wdm-property-facts"><li>Assessment —</li><li>Market —</li><li>Annual tax —</li><li>Watchdog Score —</li></ul></div><div class="wdm-property-side"><a class="wdm-property-action" href="/property/">Look up a property</a><small>Start with an address, owner, block &amp; lot, or town.</small></div></div>'+
        '<div class="wdm-tabs"><button class="wdm-tab active" type="button">Overview</button><button class="wdm-tab" type="button" data-wdm-scroll="#wdd-positions">My Properties</button><button class="wdm-tab" type="button" data-wdm-scroll="#wdd-rail">Change Alerts</button><button class="wdm-tab" type="button" data-wdm-scroll="#wd4-root">Market Insights</button><button class="wdm-tab" type="button" data-wdm-scroll="#wdd-queue">Reports &amp; Actions</button></div>'+
        '<div class="wdm-overview-body"><h2 class="wdm-section-title">Property Intelligence</h2><div class="wdm-metrics"><div class="wdm-metric"><span>Estimated Value</span><strong>—</strong><small>Add a property to begin.</small></div><div class="wdm-metric"><span>Assessment</span><strong>—</strong><small>Current public record.</small></div><div class="wdm-metric"><span>Annual Tax</span><strong>—</strong><small>Latest available bill data.</small></div><div class="wdm-metric"><span>Watchdog Score</span><strong>—</strong><small>Updates as evidence accumulates.</small></div></div></div>';
    ...
    overview.innerHTML = '<div class="wdm-property">'+
      '<img class="wdm-property-photo" src="/agent/assets/property-house.webp" alt="" width="148" height="94">'+
      '<div class="wdm-property-info"><h1>'+esc(address)+'</h1><p>'+esc(place || 'New Jersey')+'</p><span class="wdm-property-type">'+esc(propertyKind(property))+'</span><ul class="wdm-property-facts"><li>Assessment '+esc(money(property.assessed))+'</li><li>Market '+esc(money(property.watchdog_value))+'</li><li>Tax '+esc(money(property.last_year_tax))+'</li><li>Watchdog '+esc(scoreNum == null ? '—' : Math.round(scoreNum))+'</li></ul></div>'+
      '<div class="wdm-property-side"><a class="wdm-property-action" href="'+esc(openUrl)+'">Open Property</a><small>'+esc(dateLabel(last))+'</small></div></div>'+
      '<div class="wdm-tabs"><button class="wdm-tab active" type="button">Overview</button><button class="wdm-tab" type="button" data-wdm-scroll="#wdd-positions">My Properties</button><button class="wdm-tab" type="button" data-wdm-scroll="#wdd-rail">Change Alerts</button><button class="wdm-tab" type="button" data-wdm-scroll="#wd4-root">Market Insights</button><button class="wdm-tab" type="button" data-wdm-scroll="#wdd-queue">Reports &amp; Actions</button></div>'+
      '<div class="wdm-overview-body"><h2 class="wdm-section-title">Property Intelligence</h2><div class="wdm-metrics">'+
        ...
        '<div class="wdm-metric"><span>Watchdog Score</span><strong>'+(scoreNum == null ? '—' : Math.round(scoreNum))+'</strong><small class="'+scoreTone+'">'+esc(scoreNote)+'</small></div>'+
      '</div><div class="wdm-overview-bottom"><section class="wdm-chart-card"><div class="wdm-card-head"><h2>Watchdog Score History</h2><a href="/property/home">Property details</a></div>'+chartMarkup(WD, property, scoreNum)+'</section><section class="wdm-changes-card"><div class="wdm-card-head"><h2>Recent Changes</h2><a href="/property/pulse">View All</a></div>'+changesMarkup(WD, property)+'</section></div></div>';
    overview.hidden = false;
    bindDynamicTabs();
    return true;
  }
