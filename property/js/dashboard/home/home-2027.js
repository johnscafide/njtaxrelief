/* Watchdog Property Home 2027 chrome.
   Keeps the existing property intelligence engine intact and modernizes only
   application navigation, weather, notifications and profile interactions. */
(function(){
'use strict';
...
function paintDate(){
  var d=new Date(),date=document.getElementById('hm27-date-label'),day=document.getElementById('hm27-day-label');
  if(date)date.textContent=d.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'});
  if(day)day.textContent=d.toLocaleDateString('en-US',{weekday:'long'});
}
...
function ensurePopovers(){
  if(!document.getElementById('hm27-profile-pop')){
    var p=document.createElement('aside');p.id='hm27-profile-pop';p.className='hm27-pop';document.body.appendChild(p);
  }
  if(!document.getElementById('hm27-notice-pop')){
    var n=document.createElement('aside');n.id='hm27-notice-pop';n.className='hm27-pop';document.body.appendChild(n);
  }
}
function paintProfile(){ensurePopovers();var p=document.getElementById('hm27-profile-pop');if(!p||!user)return;
  p.innerHTML='<header><span><b>'+esc(profile.display_name||profile.full_name||firstName())+'</b><small>'+esc(user.email||'')+'</small></span><small>'+esc(planLabel())+'</small></header><nav>'+
...
function paintNotifications(force){ensurePopovers();var p=document.getElementById('hm27-notice-pop');if(!p)return;var read=getReadAt()?new Date(getReadAt()).getTime():0,u=unreadCount();
  p.innerHTML='<header><span><b>Property notifications</b><small>'+(u?u+' unread':'You’re caught up')+'</small></span><button class="hm27-read" type="button" data-hm27="read-all">Read all</button></header><div class="hm27-notice-list">'+(events.length?events.slice(0,20).map(function(e){var unread=(new Date(e.occurred_at).getTime()||0)>read;return'<a class="hm27-notice" href="/property/pulse"><i class="fas '+iconFor(e)+'"></i><span><b>'+esc(e.title||pretty(e.event_type||e.marker_id))+'</b><small>'+esc(property&&property.address||'Current property')+'</small></span><em>'+esc(when(e.occurred_at))+'</em></a>';}).join(''):'<div style="padding:28px 14px;text-align:center;color:#8291a7">No recent property notifications.</div>')+'</div>';
...
function loadCurrentProperty(){
  var pin=resolvePin();
  var q=db.from('saved_properties').select('id,pams_pin,kind,address,town,county,zip,lat,lon').order('created_at',{ascending:false});
  if(pin)q=q.eq('pams_pin',pin);
...
function loadWeather(){
  if(!property)return Promise.resolve();var lat=valid(property.lat),lon=valid(property.lon);if(lat==null||lon==null||!isNj(lat,lon)){setWeather(null,property.town||'New Jersey','');return Promise.resolve();}
  var token=++weatherToken;setWeather(null,property.town||'Local conditions','');
  return fetchJson('https://api.weather.gov/points/'+lat.toFixed(4)+','+lon.toFixed(4),8000).then(function(point){if(token!==weatherToken)return;var u=point&&point.properties&&point.properties.forecastHourly;if(!u)throw new Error('no forecast');return fetchJson(u,8000);}).then(function(j){if(token!==weatherToken)return;var p=j&&j.properties&&j.properties.periods&&j.properties.periods[0];if(!p)throw new Error('empty');setWeather(p.temperature,property.town||'Local conditions',p.shortForecast);}).catch(function(){
    var u='https://api.open-meteo.com/v1/forecast?latitude='+lat.toFixed(4)+'&longitude='+lon.toFixed(4)+'&current=temperature_2m,weather_code&temperature_unit=fahrenheit&timezone=auto';
    return fetch(u).then(function(r){if(!r.ok)throw new Error('weather');return r.json();}).then(function(j){if(token!==weatherToken)return;var c=j.current||{};setWeather(c.temperature_2m,property.town||'Local conditions','');}).catch(function(){setWeather(null,property.town||'Local conditions','');});
  });
}
...
function bind(){
  document.addEventListener('click',function(ev){
    var menu=ev.target.closest&&ev.target.closest('#hm27-menu');if(menu){ev.preventDefault();document.body.classList.toggle('db-sidebar-expanded');return;}
    var profileBtn=ev.target.closest&&ev.target.closest('#hm27-profile');if(profileBtn){ev.preventDefault();ev.stopPropagation();togglePop('hm27-profile-pop');return;}
    var notify=ev.target.closest&&ev.target.closest('#hm27-notify');if(notify){ev.preventDefault();ev.stopPropagation();togglePop('hm27-notice-pop');return;}
    ...
    if(document.body.classList.contains('db-sidebar-expanded')&&!ev.target.closest('#property-side-menu')&&!ev.target.closest('#hm27-menu'))document.body.classList.remove('db-sidebar-expanded');
    if(!ev.target.closest('.hm27-pop')&&!ev.target.closest('#hm27-profile')&&!ev.target.closest('#hm27-notify'))closePops();
  },true);
  document.addEventListener('keydown',function(ev){if(ev.key==='Escape'){closePops();document.body.classList.remove('db-sidebar-expanded');}});
...
function boot(){paintDate();ensurePopovers();bind();db.auth.getSession().then(function(r){var s=r&&r.data&&r.data.session;if(!s||!s.user)return;user=s.user;return db.from('profiles').select('display_name,full_name,avatar_url,plan,plan_tier,account_role').eq('id',user.id).maybeSingle();}).then(function(r){if(r&&r.data)profile=r.data;paintAvatar();paintProfile();return loadCurrentProperty();}).then(function(){return Promise.all([loadEvents(),loadWeather()]);}).catch(function(){});
  weatherTimer=setInterval(function(){if(!document.hidden)loadWeather();},20*60*1000);
}
