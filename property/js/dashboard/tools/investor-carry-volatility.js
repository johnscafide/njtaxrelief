...
(function () {
  ...
  function portfolio(list){
    ...
    return '<section class="sec"><h4><i class="fas fa-chart-area"></i> Investor Carry-Cost Volatility</h4><div class="pci-portfolio"><div class="pci-hero"><div><b>'+avg+'</b><span>/ 100 portfolio average</span></div><p>Tax-driven operating-cost uncertainty across '+vals.length+' saved propert'+(vals.length===1?'y':'ies')+'.</p></div>'+vals.slice(0,8).map(function(x){return '<a href="/property/home.html?pin='+encodeURIComponent(x.r.pams_pin||'')+'#sec-compare"><span>'+String(x.r.address||'Saved property')+'</span><b>'+x.v.score+' · '+x.v.band+'</b></a>';}).join('')+'</div></section>';
  }
