...
(function(){
  ...
  function build(r,d){
    ...
    return summary+'<div class="teg-map">'+center+'<div class="teg-nodes">'+nodes.join('')+'</div></div>'+
      ...
      '<div class="teg-actions"><button type="button" onclick="tegRefresh(\''+key(r)+'\')"><i class="fas fa-rotate"></i> Refresh evidence graph</button><button type="button" onclick="ddEvidence(\''+key(r)+'\')"><i class="fas fa-file-arrow-down"></i> Download source evidence</button><a href="/property/workbench.html"><i class="fas fa-folder-tree"></i> Open Case Workbench</a></div>';
  }
