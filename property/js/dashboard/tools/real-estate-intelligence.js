...
(function(){
  ...
  function card(r){var ms=metrics(r);return toolCard('Real Estate Professional Intelligence','fa-house-circle-check','<p class="tl-p">Ten Watchdog-derived conversation signals for agents and brokers. Each is built from attributable tax, assessment, market and municipal inputs already in this report; they do not replace MLS facts, inspections or professional diligence.</p><div class="rai-grid">'+ms.map(function(m){return '<a class="dm" data-marker-id="'+m[0]+'" data-marker-value="'+m[2]+' / 100" data-marker-note="'+m[3].replace(/"/g,'&quot;')+'" href="/property/marker?id='+encodeURIComponent(m[0])+'&value='+encodeURIComponent(m[2]+' / 100')+'&note='+encodeURIComponent(m[3])+'"><span><b>'+m[1]+'</b><small>'+m[3]+'</small></span><strong>'+m[2]+'</strong><i class="fas fa-chevron-right"></i></a>';}).join('')+'</div><div class="tl-fine">Watchdog proprietary markers. Scores are decision aids, not predictions of sale price, buyer behavior, legal outcome or appraisal. The marker detail pages publish tier, professional fit, methodology and source lineage.</div>');}
