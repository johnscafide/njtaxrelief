...
(function(){
  ...
  function calc(payload,r){
    ...
    var types={};rows.forEach(function(x){types[String(x.permittype||x.permittypedesc||'unknown')]=1;});
    var intensity=Math.min(100,Math.round(recent*14+Math.max(0,Object.keys(types).length-1)*8));
    var sale=day(r&&(r._lastSaleDate||r.last_sale_date||r.sale_date));var postSale=sale?rows.filter(function(x){var d=day(x.permitdate);return d&&d>sale;}).length:null;
    ...
    return {records:rows.length,open:open.length,stale:stale,oldest:oldest,median:median,recent:recent,intensity:intensity,postSale:postSale,score:score,band:score>=65?'High review':score>=35?'Review':'Routine'};
  }
  function render(r,payload){var host=document.getElementById('pli-'+key(r));if(!host)return;var v=calc(payload,r);host.innerHTML='<div class="pci-hero"><div><b>'+v.score+'</b><span>/ 100 lifecycle review</span></div><p><strong>'+v.band+'.</strong> '+(v.open?v.open+' permit record'+(v.open===1?' remains':'s remain')+' issued without a certificate in the state feed.':'No issued-without-certificate gap appears in the matching state feed.')+'</p></div><div class="pci-scenarios"><span><small>Matched records</small><b>'+v.records+'</b></span><span><small>Unclosed projects</small><b>'+v.open+'</b></span><span><small>Oldest open</small><b>'+v.oldest+' days</b></span><span><small>Median permit → cert.</small><b>'+(v.median==null?'—':v.median+' days')+'</b></span><span><small>Improvement intensity</small><b>'+v.intensity+'/100</b></span><span><small>Post-sale permits</small><b>'+(v.postSale==null?'sale date unavailable':v.postSale)+'</b></span></div><div class="tl-fine">Watchdog lifecycle analysis of the live NJ DCA Construction Permit feed, matched by municipality code + block + lot. Improvement intensity reflects recent permit frequency and type diversity, not construction value. A missing certificate is a review flag, not proof that work remains legally open; verify locally.</div>';}
