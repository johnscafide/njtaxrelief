(function () {
  ...
  var plans = {
    agent: {
      ...
      audience: 'For agents who want property research, opportunity discovery and client workflows.',
    ...
    pro: {
      ...
      features: ['Expanded professional workbenches', 'Advanced research workflows', 'Professional research and exports']
    },
    pro_plus: {
      internal: 'pro_plus', name: 'Pro+', monthly: 399, yearly: 3990, badge: 'MAXIMUM DATA ACCESS',
      audience: 'For power users who need deeper data, bulk research and high-volume workflows.',
  ...
  function date(value) {
    ...
    return Number.isFinite(d.getTime()) ? d.toLocaleDateString('en-US', { month:'short', day:'numeric', year:'numeric' }) : 'Not available';
  }
  ...
  function pricing(currentPlan, developer) {
    ...
    return '<section class="ac-section ac-pricing" id="membership-options">' +
      '<header class="ac-pricing-header"><div><span>MEMBERSHIP OPTIONS</span><h2>Choose the workspace you need</h2><p>Save 17% with yearly billing.</p></div>' +
  ...
  function render() {
    ...
    $('ac-app').innerHTML =
      ...
      '<section class="ac-grid"><article class="ac-card ac-plan"><span>CURRENT MEMBERSHIP</span><h2>' + planLabel(plan) + '</h2><p>' + (developer ? 'Developer access' : title(entitlement.subscription_status || 'none')) + '</p><div class="ac-meta"><div><small>Renewal / period end</small><b>' + date(entitlement.current_period_end) + '</b></div><div><small>Profession</small><b>' + esc(title(entitlement.profession || 'not set')) + '</b></div></div>' +
      ...
      '<article class="ac-card ac-account-summary"><div class="ac-account-summary-head"><span>ACCOUNT</span><h2>Watchdog Account</h2></div><div class="ac-account-rows ac-account-overview"><div class="ac-account-primary"><i class="fas fa-envelope"></i><span><small>ACCOUNT EMAIL</small><b>' + esc(user.email || '') + '</b></span></div><div class="ac-account-detail-grid"><div><i class="fas fa-right-to-bracket"></i><span><small>SIGN-IN METHODS</small><b id="ac-account-signins">Checking…</b></span></div><div><i class="fas fa-calendar-day"></i><span><small>MEMBER SINCE</small><b>' + date(user.created_at) + '</b></span></div><div><i class="fas fa-circle-check"></i><span><small>ACCOUNT STATUS</small><b>Active</b></span></div><div class="ac-account-id-row"><i class="fas fa-fingerprint"></i><span><small>ACCOUNT ID</small><b class="ac-account-id"><code>' + esc(String(user.id || '').slice(0,8)) + '…</code><button type="button" data-copy-account-id="' + esc(user.id || '') + '" aria-label="Copy account ID"><i class="fas fa-copy"></i></button></b></span></div></div></div></article></section>' +
