  ...
  function open(options) {
    ...
    options.modal('Verify you own this home',
      ...
      '<div class="pl-form" style="grid-template-columns:1fr;">' +
        '<button id="njptr-verify-request" type="button">Request postcard code</button>' +
      '</div>' +
      '<div class="auth-or"><span>already have a code</span></div>' +
      '<div class="pl-form" style="grid-template-columns:1fr;">' +
        '<input id="njptr-verify-code" type="text" placeholder="Six character code" maxlength="8" autocomplete="one-time-code" style="text-transform:uppercase;letter-spacing:.15em;">' +
        '<button id="njptr-verify-redeem" type="button">Verify ownership</button>' +
      '</div>' +
      '<button id="njptr-verify-later" class="plm-rbtn" type="button" style="margin-top:12px;">Not now</button>' +
      '<div class="auth-fine">Choosing Not now keeps the property saved as your home, but it remains marked unverified until you enter the postcard code.</div>',
      true);
    ...
    if (requestButton) requestButton.addEventListener('click', function () {
      ...
      requestButton.textContent = 'Creating secure code...';
      client.functions.invoke('request-verify-code', {
        body: { pams_pin: options.pin, address_line1: address, city: town, postal_code: zip }
      }).then(function (result) {
        var data = (result && result.data) || {};
        if (result.error || !data.ok) {
          var reason = data.reason || (result.error && result.error.message) || 'Could not request a code';
          requestButton.disabled = false;
          requestButton.textContent = 'Request postcard code';
          console.error('Verification request failed:', data.stage || 'request', result.error || data);
          if (typeof options.toast === 'function') options.toast(reason);
          return;
        }
        options.modal('Postcard request received',
          '<p>Your secure code was sent to our mailing desk. We will mail it to <b>' + text(address) + '</b>.</p>' +
          '<p>Allow a few days for it to arrive, then return to this property and choose Verify ownership to enter the code.</p>' +
          '<button id="njptr-verify-done" class="plm-rbtn" type="button">Got it</button>', true);
        var done = document.getElementById('njptr-verify-done');
    ...
    function redeem() {
      ...
      if (!code) {
        if (typeof options.toast === 'function') options.toast('Enter the code');
        return;
      }
      ...
      client.rpc('redeem_verify_code', { p_pin: options.pin, p_code: code }).then(function (result) {
        ...
        if (result.error || !data.ok) {
          ...
          if (typeof options.toast === 'function') options.toast(data.reason === 'wrong code' ? 'That code did not match' : (data.reason || 'Could not verify'));
          return;
        }
        options.modal('Verified', '<p>This home is now marked as verified. Thanks for confirming.</p>' +
          '<button id="njptr-verify-close" class="plm-rbtn" type="button">Close</button>', true);
        var closeButton = document.getElementById('njptr-verify-close');
    ...
    if (codeInput) codeInput.addEventListener('keydown', function (event) {
