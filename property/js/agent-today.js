...
async function load(){
 try{var a=await db.auth.getUser();user=a&&a.data&&a.data.user;if(!user)return;
  ...
  tasks.sort(function(a,b){return a.p-b.p});var q=$('#ad-today-queue');q.innerHTML=tasks.length?tasks.slice(0,7).map(function(t){return task(t.icon,t.title,t.sub,t.href,t.tag)}).join(''):'<div class="ad-today-empty">No dated or flagged work is waiting. Your sourced Opportunity Desk remains below.</div>';
 }catch(e){var q=$('#ad-today-queue');if(q)q.innerHTML='<div class="ad-today-empty">Today’s cross-workspace summary could not load. Your existing Opportunity Desk is still available below.</div>'}
}
