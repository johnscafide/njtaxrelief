...
var EVENTS=new Set([
 'page_view','tool_open','marker_viewed','property_lookup_started','property_lookup_succeeded','property_saved','monitoring_enabled','export_started','export_completed','upgrade_cta_clicked','checkout_started','subscription_confirmed',
 ...
 'data_center_tab_viewed','data_center_searched','data_center_filtered','data_center_field_selected','data_center_build_started','data_center_dataset_built','data_center_view_saved','data_center_export_completed','data_center_delivery_scheduled'
]);
...
var visitor=safeStore(localStorage,'wd_visitor_id',uuid),session=safeStore(sessionStorage,'wd_session_id',uuid),sentPage=false,seen=new Set(),accessToken='',heartbeatTimer=null,authSubscription=null;
...
function scrubProps(obj){var out={},allowed=['marker_id','plan','tool','action','format','source','result_count_bucket','status','billing_period','tier','surface','interaction','model','intent_status','queue_state','reason_count_bucket','scope','selected_count_bucket','row_count_bucket','filter'];obj=obj||{};allowed.forEach(function(k){if(obj[k]!=null)out[k]=clean(obj[k],100)});return out}
function syncAccessToken(sessionValue){accessToken=clean(sessionValue&&sessionValue.access_token,5000)}
async function resolveAuth(){try{if(!window.NJPTRSupabaseRuntime||typeof window.NJPTRSupabaseRuntime.createClient!=='function')return;var client=window.NJPTRSupabaseRuntime.createClient(),res=await client.auth.getSession();syncAccessToken(res&&res.data&&res.data.session);if(!authSubscription&&client.auth&&typeof client.auth.onAuthStateChange==='function'){var listener=client.auth.onAuthStateChange(function(event,nextSession){syncAccessToken(nextSession);if((event==='SIGNED_IN'||event==='TOKEN_REFRESHED'||event==='INITIAL_SESSION')&&nextSession&&sentPage)setTimeout(heartbeat,0)});authSubscription=listener&&listener.data&&listener.data.subscription||listener||true}}catch(_){accessToken=''}}
