(function(){
  ...
  function render(){
    ...
    status.textContent=list.length+(all.length>250?' of '+all.length:'')+' matching entries'+(state.query?' for “'+state.query+'”':'');
    ...
    results.innerHTML=list.map(function(entry){var badgeClass=entry.owner==='JS'?' js':'';var line=entry.line?' · line '+entry.line:'';var github=entry.file.indexOf('property/')===0?GH+entry.file+(entry.line?'#L'+entry.line:''):'';return '<article class="cg-result"><div class="cg-result-top"><div class="cg-text">'+highlight(entry.text,state.query)+'</div><span class="cg-badge'+badgeClass+'">'+esc(entry.owner)+'</span></div><div class="cg-meta"><span class="cg-path">'+highlight(entry.file,state.query)+'</span><span class="cg-line">'+esc(entry.contentClass)+line+'</span></div><div class="cg-guidance">'+esc(entry.guidance)+'</div><div class="cg-actions">'+(github?'<a href="'+esc(github)+'" target="_blank" rel="noopener"><i class="fa-brands fa-github"></i> Open source</a>':'')+'<button type="button" data-copy="'+esc(entry.file+(entry.line?':'+entry.line:''))+'"><i class="fas fa-copy"></i> Copy location</button></div></article>';}).join('');
  }
  function setMetrics(data){document.getElementById('cg-entry-count').textContent=(data.entryCount||0).toLocaleString();document.getElementById('cg-file-count').textContent=(data.sourceFileCount||0).toLocaleString();document.getElementById('cg-html-count').textContent=((data.owners||{}).HTML||0).toLocaleString();document.getElementById('cg-js-count').textContent=((data.owners||{}).JS||0).toLocaleString();}
  fetch('/property/developer/content-glossary/glossary.json',{cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('Glossary data unavailable');return res.json();}).then(function(data){state.entries=Array.isArray(data.entries)?data.entries:[];setMetrics(data);render();}).catch(function(error){status.textContent='Glossary could not load: '+error.message;
    // content-architecture: dynamic — this diagnostic appears only when the generated build artifact is unavailable.
    results.innerHTML='<div class="cg-empty">The generated glossary is unavailable on this build. Run <code>npm run content-glossary:generate</code>.</div>';});
