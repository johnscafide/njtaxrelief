(function(){
  ...
  Promise.all([
    ...
    fetch('/property/data/marker-registry.json?v=20260807c').then(function(r){return r.json();}),
    ...
    fetch('/property/data/derived-marker-formulas.json?v=20260807c').then(function(r){return r.json();}).catch(function(){return{markers:{}};}),
    fetch('/property/data/marker-refresh-policy.json?v=20260807c').then(function(r){return r.json();}).catch(function(){return{markers:[]};})
  ]).then(function(x){
    ...
    $('mk-why').textContent=rich.why||m.professional_reason||('This '+label(m.scope).toLowerCase()+' marker is used in Watchdog '+label(m.category).toLowerCase()+' analysis and can be selected in the Pro+ Data Center.');
