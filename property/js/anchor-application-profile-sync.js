...
var APP_KEY='wd_anchor_2025_application_id',panel=null,busy=false,autoSynced='';
...
function identityPayload(){return{p_application_id:appId(),p_first_name:field('applicant.first')||null,p_middle_name:field('applicant.middle')||null,p_last_name:field('applicant.last')||null,p_address:null,p_city:null,p_state:null,p_zip:null,p_municipality_code:null}}
...
async function syncIdentity(){var id=appId();if(!id||autoSynced===id)return;var data=identityPayload();if(!data.p_first_name&&!data.p_last_name)return;autoSynced=id;try{var result=await db.rpc('set_my_reusable_profile_v1',data);if(result.error)throw result.error}catch(_){autoSynced=''}}
async function save(){if(busy)return;var id=appId(),button=q('#wd-anchor-profile-save-button',panel);if(!id){status('missing','error');return}busy=true;button.disabled=true;status('saving');try{var result=await db.rpc('set_my_reusable_profile_v1',payload());if(result.error)throw result.error;autoSynced=id;status('saved','success');button.textContent=copy('saved_button')}catch(error){status('failed','error');button.disabled=false}finally{busy=false}}
...
function sync(){if(complete.classList.contains('is-active')){syncIdentity();mount()}}
