...
var CATALOG={
  ...
  agent:{name:'Agent',eyebrow:'AGENT WORKFLOW',promise:'Find property changes worth a client conversation.',bestFor:'Real estate professionals working buyers, sellers, a sphere or a farm.',outcomes:['Prioritize properties instead of working a flat list','Use property signals as reasons to follow up','Move research into reports, outreach and next actions']},
  ...
  teams:{name:'Teams',eyebrow:'TEAM WORKSPACE',promise:'Make Watchdog research repeatable across a team.',bestFor:'Organizations that need shared access, administration and higher-volume workflows.',outcomes:['Coordinate Intelligence across a team','Manage shared work and member access','Standardize high-volume research across professionals']},
...
var OUTCOMES={property_baseline:{minimum:'standard',label:'Understand and monitor a property'},agent_opportunity:{minimum:'agent',label:'Turn property changes into agent opportunities'},property_decision:{minimum:'pro',label:'Run evidence-backed property-level professional Intelligence'},population_triage:{minimum:'pro_plus',label:'Find and prioritize patterns across many properties'},team_operations:{minimum:'teams',label:'Coordinate Intelligence across an organization'}};
var ROBUST=[
...
function proPricing(){
  ...
  if(compare&&compare.dataset.outcomeEnhanced!=='1'){
    ...
    if(h)h.textContent='Compare plans';
    if(p)p.textContent='Choose the level that matches the work you need to do.';
  ...
  if(platform&&platform.dataset.intelligenceLive!=='1'){
    ...
    Array.prototype.slice.call(platform.querySelectorAll('li')).forEach(function(li){if(/Planned AI-assisted change monitoring/i.test(li.textContent||''))li.innerHTML='<i class="fas fa-check"></i> Property-change monitoring with evidence-backed findings'});
  ...
  if(note&&note.dataset.intelligenceLive!=='1'){
    note.textContent='Watchdog Intelligence is live on eligible plans. Preview models remain clearly labeled.';
...
function renderProShots(){
  ...
  var copy=[
    ['01 / Property workspace','Keep property research<br>in one place.','See the property image, assessment, tax context, Watchdog Score and next actions together.'],
    ...
    ['04 / Professional workspace','Build the working file.','Data Workbench brings property data and professional research into one workspace.'],
    ['05 / Watchdog Intelligence + Voice','Ask a question.<br>Hear the brief.','Use Watchdog Intelligence for sourced property questions, analysis and Voice.']
  ];
