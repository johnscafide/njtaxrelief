(function(){
  ...
  var client=null,busy=false;
  function sb(){if(!client&&window.supabase)client=window.supabase.createClient(URL,KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,flowType:'pkce',storageKey:'sb-uvkvaxljhhngydvlrzom-auth-token'}});return client}
  function product(v){var x=String(v||'').toLowerCase();if(x==='agent')return{tier:'agent',plan:'agent'};if(x==='pro')return{tier:'pro',plan:'pro'};if(x==='pro_plus'||x==='pro+')return{tier:'pro_plus',plan:'pro_plus'};if(x==='teams')return{tier:'teams',plan:'teams',monthlyOnly:true};return null}
  ...
  function resumeLifetime(){var tier='';try{tier=String(sessionStorage.getItem('watchdog:lifetime:pending')||'')}catch(e){}if(!product(tier)||tier==='teams')return false;session().then(function(s){if(!s)return;try{sessionStorage.removeItem('watchdog:lifetime:pending')}catch(e){}busy=true;invoke('create-lifetime-checkout',{tier:tier}).then(function(p){if(!p||!p.url)throw new Error('Stripe did not return a secure Founding Lifetime checkout URL.');location.href=p.url}).catch(function(e){busy=false;console.error('Founding Lifetime resume failed',e);if(e.code==='LIFETIME_ACTIVE_SUBSCRIPTION')toast('This account already has recurring billing. Manage that subscription before switching to Lifetime.');else if(e.code==='LIFETIME_ALREADY_ACTIVE')toast('Founding Lifetime is already active on this account.');else if(e.code==='WATCHDOG_TEST_NO_REAL_SPEND')toast('This test account cannot create a real charge.');else toast('Lifetime checkout error: '+((e&&e.message)||'Please try again.'))})});return true}
  function resume(){if(resumeLifetime())return;var p=null;try{p=JSON.parse(sessionStorage.getItem('watchdog:billing:pending')||'null')}catch(e){}if(!p||!p.tier)return;session().then(function(s){if(!s)return;try{sessionStorage.removeItem('watchdog:billing:pending')}catch(e){}busy=false;checkout(p.tier,{cadence:p.cadence})})}
...
(function(){var p=(location.pathname||'').replace(/\/+$/,'');if((p==='/property/pro'||p==='/pro')&&!document.querySelector('script[src="/property/js/shared-nav-address-autocomplete.js"]')){var s=document.createElement('script');s.src='/property/js/shared-nav-address-autocomplete.js';s.defer=true;document.head.appendChild(s)}})();
(function(){var p=(location.pathname||'').replace(/\/+$/,'');if((p==='/property/pro'||p==='/pro')&&!document.querySelector('script[src="/property/js/pro-buyer-reassurance.js"]')){var s=document.createElement('script');s.src='/property/js/pro-buyer-reassurance.js';s.defer=true;document.head.appendChild(s)}})();
(function(){var p=(location.pathname||'').replace(/\/+$/,'');if((p==='/property/pro'||p==='/pro')&&!document.querySelector('script[src^="/property/js/pro-first-run.js"]')){var s=document.createElement('script');s.src='/property/js/pro-first-run.js?v=20260828a';s.defer=true;document.head.appendChild(s)}})();
(function(){var p=(location.pathname||'').replace(/\/+$/,'');if(p!=='/property/pro'&&p!=='/pro')return;if(!document.getElementById('wd-landing-intelligence-css')){var l=document.createElement('link');l.id='wd-landing-intelligence-css';l.rel='stylesheet';l.href='/property/css/landing-intelligence.css';document.head.appendChild(l)}if(document.getElementById('wd-landing-intelligence-loader'))return;var s=document.createElement('script');s.id='wd-landing-intelligence-loader';s.src='/property/js/landing-intelligence.js';s.async=false;document.body.appendChild(s)})();