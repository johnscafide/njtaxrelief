...
function installMunicipalityLookup(){
  ...
  button.addEventListener('click',async function(){if(!hasMainAddress()){status.textContent=copyText('wd-municipality-missing-address');status.classList.add('is-error');return;}button.disabled=true;status.classList.remove('is-error');status.textContent=copyText('wd-municipality-looking-up');result.hidden=true;try{var subject=await lookupMainProperty(),code=String(subject.pamsPin||'').replace(/\D/g,'').slice(0,4);if(!/^\d{4}$/.test(code))throw new Error('code');pendingCode=code;codeOut.textContent=code;nameOut.textContent=[subject.municipality,subject.county].filter(Boolean).join(' · ');result.hidden=false;status.textContent='Watchdog matched this address to a New Jersey municipality. Confirm the code below.';}catch(err){status.textContent=err&&err.message==='missing_address'?copyText('wd-municipality-missing-address'):copyText('wd-municipality-not-found');status.classList.add('is-error');}finally{button.disabled=false;}});
  use.addEventListener('click',function(){if(!pendingCode)return;input.value=pendingCode;emit(input);status.classList.remove('is-error');status.textContent='Code '+pendingCode+' added. Review it before continuing.';});
}
function installParcelGuidance(){
  ...
  function paintTaxMatch(target,subject){if(!target||!subject)return;var box=q('[data-tax-match]',target),value=q('[data-tax-value]',target),caption=q('[data-tax-caption]',target);if(subject.lastYearTax!==undefined&&subject.lastYearTax!==null&&Number(subject.lastYearTax)>=0){box.hidden=false;value.textContent=money(subject.lastYearTax);caption.textContent='Latest prior-year tax amount in the matched NJ parcel record. Use this only as a comparison and verify the exact year on your municipal bill.';}}
  if(taxCard){var taxButton=q('[data-tax-lookup]',taxCard),taxStatus=q('[data-tax-status]',taxCard);taxButton.addEventListener('click',async function(){if(!hasMainAddress()){taxStatus.textContent=copyText('wd-parcel-missing-address');return;}taxButton.disabled=true;taxStatus.textContent='Checking the matched Watchdog parcel record...';try{var subject=propertyMatch||await lookupMainProperty();paintTaxMatch(taxCard,subject);taxStatus.textContent=subject&&subject.lastYearTax!=null?'Reference found. Compare it with your municipal tax bill before entering a year amount.':'No tax reference was available. Use your municipal tax bill or tax collector account.';}catch(_){taxStatus.textContent='Watchdog could not confidently match a tax record. Use your municipal tax bill or tax collector account.';}finally{taxButton.disabled=false;}});}
}
