...
(function(){
  ...
  var obs=new MutationObserver(function(){
    clearTimeout(window.__njwV3Scan);
    window.__njwV3Scan=setTimeout(scan,90);
    // Pure debounce can starve scan() forever on a page with continuous
    // mutations (e.g. the live map redrawing). Guarantee it still runs at
    // least every 500ms no matter how busy the page is.
    if(!__njwV3MaxWait){
      __njwV3MaxWait=setTimeout(function(){
        __njwV3MaxWait=null;
        clearTimeout(window.__njwV3Scan);
        scan();
      },500);
