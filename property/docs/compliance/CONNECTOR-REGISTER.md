...
- **Scopes/permissions and least-privilege justification:** The initial direct connector records and exercises only `crm.contacts.read`. Activity, note and CRM-write scopes are not granted or exercised in this version and require separate implementation and review. `intelligence.context.read` is separate, optional and disabled by default.
...
- **Security/assurance evidence reviewed:** Server-only provider tables with RLS and revoked browser grants; Vault-backed provider and worker credentials; provider credential validation before storage; fixed Teams entitlement; incremental cursor design; service-role-only batch normalizer; database context-key allowlist; auditable sync runs; disconnect purge path; invalid-worker-token 401 test; database allowlist smoke test; Supabase Security Advisor review.
