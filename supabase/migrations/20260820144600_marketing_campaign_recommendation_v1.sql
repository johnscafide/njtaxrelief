-- NJW-253 Marketing Studio Phase F: governed aggregate campaign recommendation v1.
-- Recommends campaign messaging from aggregate Watchdog Intelligence only.
-- It never changes audience membership, launches mail, purchases postage, charges payment, or approves production proof.
...
grant select on public.marketing_campaign_recommendations to authenticated;
revoke insert, update, delete, truncate, references, trigger on table public.marketing_campaign_recommendations from anon, authenticated;
...
begin
  ...
  if r.id is null then return jsonb_build_object('exists',false,'campaign_id',p_campaign_id,'policy_version','marketing-campaign-play-v1'); end if;
  select e into selected
  from jsonb_array_elements(r.candidate_scores) e
  where e->>'play_key'=coalesce(r.decision_play_key,r.recommended_play_key)
  limit 1;
  return jsonb_build_object(
    ...
    'recommended_play_key',r.recommended_play_key,'recommended_label',r.recommended_label,'recommended_goal',r.recommended_goal,
    'creative_direction',r.creative_direction,'rationale',r.rationale,'aggregate_summary',r.aggregate_summary,'candidate_scores',r.candidate_scores,
    'source_finding_count',cardinality(r.source_finding_ids),'source_run_count',cardinality(r.source_run_ids),'source_signal_ids',to_jsonb(r.source_signal_ids),
    'recommendation_confidence',r.recommendation_confidence,'evidence_coverage',r.evidence_coverage,'facts_hash',r.facts_hash,
    'decision_play_key',r.decision_play_key,'decision_reason',r.decision_reason,'decided_at',r.decided_at,
    'selected_candidate',selected,'created_at',r.created_at,'updated_at',r.updated_at,
...
begin
  ...
  from (
    ...
    where f.user_id=uid and f.opportunity_type in ('assessment_review','closing_review','change_intelligence')
  ...
  select coalesce(array_agg(id order by score desc,confidence desc) filter(where rn<=200),'{}'::uuid[]) into finding_ids
  from (select id,score,confidence,row_number() over(order by score desc,confidence desc,created_at desc) rn from tmp_campaign_recommendation_findings) z;
  select coalesce(array_agg(distinct run_id),'{}'::uuid[]) into run_ids from tmp_campaign_recommendation_findings where run_id is not null;
  select coalesce(array_agg(distinct signal_id order by signal_id),'{}'::text[]) into signal_ids
  from (
    select nullif(trim(e->>'signal_id'),'') signal_id
  ...
  ) s where signal_id is not null;
  if cardinality(signal_ids)>60 then signal_ids:=signal_ids[1:60]; end if;

  a_eligible:=norm_prof in ('agent','attorney','appraiser','investor');
  c_eligible:=norm_prof in ('agent','attorney','lender');
  x_eligible:=norm_prof in ('agent','attorney','appraiser','investor','lender');
  ...
  order by (e->>'score')::numeric desc,
           case e->>'play_key' when fallback_key then 1 else 0 end,
           e->>'play_key'
  limit 1;
  ...
  reasons:=jsonb_build_array(
    'Recommendation is based on aggregate governed campaign evidence, not an individual homeowner prediction.',
    format('%s was the highest eligible deterministic policy score at %s.',rec_label,round(rec_rank,1)),
    case when finding_count=0 then 'No eligible Watchdog Intelligence findings were available, so the campaign-goal fallback was used.' else format('%s governed findings across %s campaign properties informed the aggregate comparison.',finding_count,scope_count) end,
    'Audience membership is unchanged by this recommendation.'
  );
  manifest:=jsonb_build_object('policy_version',policy,'campaign_id',c.id,'audience_snapshot_id',c.audience_snapshot_id,'audience_snapshot_hash',snap.snapshot_hash,'original_goal',c.goal,'profession',norm_prof,'opportunity_play',c.settings->>'opportunity_play','aggregates',aggregates,'candidates',candidates);
  ...
  values(uid,c.id,'campaign.recommendation_generated','watchdog_intelligence',jsonb_build_object('recommendation_id',rid,'policy_version',policy,'recommended_play_key',rec_key,'recommended_goal',rec_goal,'recommendation_confidence',round(rec_conf,2),'evidence_coverage',round(rec_cov,2),'scope_property_count',scope_count,'finding_count',finding_count,'source_signal_count',cardinality(signal_ids),'audience_membership_changed',false,'auto_launch_enabled',false));
...
begin
  ...
  if decision='accept' then chosen:=r.recommended_play_key;next_status:='accepted';
    ...
    select e into candidate from jsonb_array_elements(r.candidate_scores) e where e->>'play_key'=chosen and coalesce((e->>'eligible')::boolean,false) limit 1;
    if candidate is null then raise exception 'Override play is not an eligible governed candidate'; end if;
    next_status:='overridden';
  else chosen:=null;next_status:='dismissed'; end if;
  ...
  if chosen is not null then
    select e into candidate from jsonb_array_elements(r.candidate_scores) e where e->>'play_key'=chosen limit 1;
  else candidate:=null; end if;
  dm:=jsonb_build_object(
    'id',r.id,'policy_version',r.policy_version,'status',next_status,'recommended_play_key',r.recommended_play_key,
    ...
    'decision_reason',reason,'decided_at',now(),'audience_membership_changed',false,'auto_launch_enabled',false,'proof_approval_bypassed',false
  );
  update public.marketing_campaigns set settings=
  ...
  insert into public.marketing_events(user_id,campaign_id,event_type,source,payload)
  values(uid,p_campaign_id,'campaign.recommendation_decision','watchdog',jsonb_build_object('recommendation_id',r.id,'decision',decision,'status',next_status,'recommended_play_key',r.recommended_play_key,'selected_play_key',chosen,'reason',reason,'audience_membership_changed',false,'auto_launch_enabled',false));
