create table if not exists public.integration_automation_proof_references (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  proof_id uuid not null references public.integration_automation_proofs(id) on delete cascade,
  proof_reference text not null unique check (proof_reference ~ '^wdp_[0-9a-f]{32}$'),
  proof_digest text not null check (proof_digest ~ '^[0-9a-f]{64}$'),
  disclosure_scope text not null default 'external_minimal' check (disclosure_scope in ('external_minimal','watchdog_internal')),
  created_at timestamptz not null default now(),
  unique(user_id,proof_id,disclosure_scope)
);
create index if not exists integration_automation_proof_refs_user_created_idx on public.integration_automation_proof_references(user_id,created_at desc);
alter table public.integration_automation_proof_references enable row level security;
revoke all on public.integration_automation_proof_references from anon,authenticated;
grant all on public.integration_automation_proof_references to service_role;
