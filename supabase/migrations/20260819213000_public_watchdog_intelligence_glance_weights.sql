...
select jsonb_build_object(
  ...
  'methodology', jsonb_build_object(
    ...
    'score_components', jsonb_build_array(
      jsonb_build_object('label','Tax burden','weight',30),
      jsonb_build_object('label','Chapter 123 fairness','weight',20),
      jsonb_build_object('label','Assessment uniformity','weight',15),
      jsonb_build_object('label','Revaluation stability','weight',15),
      jsonb_build_object('label','Property trajectory','weight',10),
      jsonb_build_object('label','Appeal recourse','weight',10)
    ),
    'missing_input_rule','Missing inputs are dropped and the remaining weights are renormalized.',
