...
begin
  ...
  return jsonb_build_object(
    ...
    'campaigns',(select coalesce(jsonb_agg(to_jsonb(c) order by c.updated_at desc),'[]'::jsonb) from (
      select mc.id,mc.name,mc.goal,mc.status,mc.budget_cents,mc.spend_cap_cents,mc.updated_at,mc.automation_state,mc.kill_switch,
        coalesce(ms.property_count,0) as audience_count,
        (select count(*) from public.marketing_campaign_steps st where st.campaign_id=mc.id) as step_count,
      ...
      from public.marketing_campaigns mc left join public.marketing_audience_snapshots ms on ms.id=mc.audience_snapshot_id
