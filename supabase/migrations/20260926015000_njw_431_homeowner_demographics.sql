-- NJW-431: expand private homeowner context without changing entitlement or Intelligence targeting.

alter table public.watchdog_onboarding_profiles
  add column if not exists household_composition text,
  add column if not exists residence_tenure_band text,
  add column if not exists primary_residence boolean;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.watchdog_onboarding_profiles'::regclass
      and conname='watchdog_onboarding_profiles_household_composition_check'
  ) then
    alter table public.watchdog_onboarding_profiles
      add constraint watchdog_onboarding_profiles_household_composition_check
      check (
        household_composition is null or household_composition in
        ('single','couple','family_children','multigenerational','roommates','other','prefer_not')
      );
  end if;
  ...
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.watchdog_onboarding_profiles'::regclass
      and conname='watchdog_onboarding_profiles_residence_tenure_band_check'
  ) then
    alter table public.watchdog_onboarding_profiles
      add constraint watchdog_onboarding_profiles_residence_tenure_band_check
      check (
        residence_tenure_band is null or residence_tenure_band in
        ('under_1','1_3','4_7','8_15','16_plus','prefer_not')
      );
  end if;
end $$;
...
declare
  ...
  v_household_composition text;
  v_residence_tenure text;
  v_primary_residence boolean;
  v_zip text;
  v_time_horizon text;
  v_years text;
  v_volume text;
...
begin
  ...
  v_household_composition := nullif(btrim(payload->>'household_composition'),'');
  v_residence_tenure := nullif(btrim(payload->>'residence_tenure_band'),'');
  ...
  if nullif(payload->>'primary_residence','') is not null then
    begin
      v_primary_residence := (payload->>'primary_residence')::boolean;
    exception when others then raise exception 'Invalid primary residence value'; end;
  end if;
  ...
  if v_household_composition is not null and v_household_composition not in ('single','couple','family_children','multigenerational','roommates','other','prefer_not') then raise exception 'Invalid household composition'; end if;
  if v_residence_tenure is not null and v_residence_tenure not in ('under_1','1_3','4_7','8_15','16_plus','prefer_not') then raise exception 'Invalid residence tenure'; end if;
  ...
  v_snapshot := jsonb_build_object(
    ...
    'household_composition',v_household_composition,
    'residence_tenure_band',v_residence_tenure,
    'primary_residence',v_primary_residence,
    'location_zip',v_zip,
  ...
  -- Demographic/private household context intentionally stays out of Intelligence assumptions.
  if v_intel then
...
comment on column public.watchdog_onboarding_profiles.household_composition is
  'Optional user-confirmed private household context; excluded from Intelligence targeting assumptions.';
comment on column public.watchdog_onboarding_profiles.residence_tenure_band is
  'Optional user-confirmed duration at current residence; excluded from Intelligence targeting assumptions.';
comment on column public.watchdog_onboarding_profiles.primary_residence is
  'Optional user-confirmed primary-residence flag; excluded from Intelligence targeting assumptions.';
