...
begin
  ...
  return jsonb_build_object(
    ...
    'summary',jsonb_build_object(
      ...
      'headline',case when v_total=0 then 'No shadow policies to calibrate' when v_ready>0 then v_ready||' policy'||case when v_ready=1 then '' else 'ies' end||' ready for counterfactual review' else 'Watchdog is still learning from measured outcomes' end
    ),
    'policies',v_policies,
