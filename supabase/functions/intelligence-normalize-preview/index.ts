...
const ENGINE_VERSION = "watchdog-intelligence-normalize-preview-v4-context-lineage";
...
function percentileMidrank(values: number[], raw: number, lowQ = 0, highQ = 1) {
  ...
  for (const v of sorted) {
    const w = Math.max(lo, Math.min(hi, v));
    if (w < x) lower++;
    else if (w === x) equal++;
  }
  return {
...
function transform(def: any, rawValue: unknown, cohortValues: unknown[], cohortDef: any, rawMap: Record<string, unknown>) {
  ...
  if (guardKey) {
    ...
    if (guardValue === null) {
      return {
        ...
        score: null,
        detail: {
          guard_source_key: guardKey,
          reason: clean(cfg.guard_reason, 240) || "Guard evidence is missing",
        },
      };
    }
    ...
    if ((guardMin !== null && guardValue < guardMin) || (guardMax !== null && guardValue > guardMax)) {
      return {
        ...
        detail: {
          ...
          guard_min: guardMin,
          guard_max: guardMax,
          reason: clean(cfg.guard_reason, 240) || "Feature guard failed",
        },
      };
    }
  }
  ...
  if (def.transform_type === "inverse_0_100") {
    const n = numeric(rawValue);
    return n === null ? { status: "missing", score: null, detail: {} } : { status: "available", score: clamp(100 - clamp(n)), detail: {} };
  }
  if (def.transform_type === "count_cap") {
    const n = numeric(rawValue);
    const cap = Math.max(Number(cfg.cap || 1), 1);
    return n === null ? { status: "missing", score: null, detail: {} } : { status: "available", score: clamp((Math.max(n, 0) / cap) * 100), detail: { cap } };
  }
  if (def.transform_type === "distance_from_target") {
    ...
    return n === null ? { status: "missing", score: null, detail: {} } : { status: "available", score: clamp((Math.abs(n - target) / delta) * 100), detail: { target, full_attention_delta: delta } };
  }
  if (def.transform_type === "numeric_decay") {
    const value = numeric(rawValue);
    if (value === null) return { status: "missing", score: null, detail: {} };
  ...
  if (def.transform_type === "categorical_map") {
    const key = clean(rawValue, 80).toLowerCase();
    const mapped = numeric(cfg[key]);
    return mapped === null ? { status: key ? "unmapped_category" : "missing", score: null, detail: { category: key || null } } : { status: "available", score: clamp(mapped), detail: { category: key } };
  }
  if (def.transform_type === "recency_decay") {
    const time = new Date(String(rawValue || ""));
    if (!Number.isFinite(time.getTime())) return { status: "missing", score: null, detail: {} };
  ...
  if (def.transform_type === "cohort_percentile_high") {
    ...
    const values = (Array.isArray(cohortValues) ? cohortValues : []).map((v) => numeric(v)).filter((x): x is number => x !== null);
    ...
    return {
      ...
      detail: {
        ...
        winsorized_low: result.low,
        winsorized_high: result.high,
        cohort_key: def.cohort_key,
        cohort_version: def.cohort_version,
      },
    };
  }
...
function normalizeContext(value: unknown) {
  ...
  return {
    municipality: clean(v.municipality ?? v.town, 140) || null,
    county: clean(v.county, 120) || null,
    property_class: clean(v.property_class ?? v.prop_class, 60) || null,
    zip: clean(v.zip, 20) || null,
    block: clean(v.block, 60) || null,
    lot: clean(v.lot, 60) || null,
  };
}
...
Deno.serve(async (req: Request) => {
  ...
  let body: any;
  try { body = await req.json(); } catch (_) { return json(req, 400, { error: "Invalid JSON" }); }
  ...
  const { data: allDefs, error: defError } = await admin
    .from("intelligence_feature_versions")
    .select("feature_key,version,source_key,label,transform_type,cohort_key,cohort_version,config,direction,status,explanation")
  ...
  if (cohortKeys.length) {
    const r = await admin
      ...
      .select("cohort_key,version,label,description,dimensions,minimum_sample_size,fallback_chain,status")
  ...
  for (const [key, def] of defs) {
    normalizationManifest[key] = {
      version: def.version,
      source_key: def.source_key,
      transform_type: def.transform_type,
      status: def.status,
      direction: def.direction,
      guard_source_key: def.config?.guard_source_key || null,
      guard_min: def.config?.guard_min ?? null,
      guard_max: def.config?.guard_max ?? null,
    };
    if (def.cohort_key) {
      const c = cohortMap.get(`${def.cohort_key}:${def.cohort_version}`);
      if (c) cohortManifest[def.cohort_key] = {
        version: c.version,
  ...
  const output = candidates.map((candidate) => {
    ...
    for (const featureKey of requested) {
      ...
      if (!def) {
        features[featureKey] = { status: "definition_missing", score: null };
        continue;
      }
      ...
      let result: any;
      try {
        result = transform(def, rawValue, cohorts[featureKey] || cohorts[sourceKey] || [], cohortDef, raw);
      } catch (e) {
        result = { status: "transform_error", score: null, detail: { error: clean((e as any)?.message || e, 300) } };
      }

      const meta = sourceMeta[sourceKey] || sourceMeta[featureKey] || {};
      const dependencies = Array.isArray(meta.dependencies) ? meta.dependencies.map((x) => clean(x, 140)).filter(Boolean).slice(0, 40) : [];
      const lineage = {
        status: clean(meta.status, 80) || null,
        provider_kind: clean(meta.provider_kind, 120) || null,
        engine_version: clean(meta.engine_version, 160) || null,
        formula: clean(meta.formula, 1000) || null,
        dependencies,
      };
      features[featureKey] = {
