...
Deno.serve(async(req:Request)=>{
  ...
  const {data:routes}=dists.length?await admin.from("transaction_provider_registry").select("jurisdiction_key,jurisdiction_name,county,provider_key,provider_label,provider_url,evidence_families,access_mode,adapter_status,source_url,last_verified_at,metadata").eq("jurisdiction_type","municipality").eq("provider_key","municipal_code_enforcement").in("jurisdiction_key",dists):{data:[]};
  const routeMap=new Map<string,Row>();for(const r of routes||[])if(r.provider_url)routeMap.set(String(r.jurisdiction_key),r);
  ...
  for(const tx of txs||[]){
    const d=district(tx),item=itemMap.get(tx.id),route=routeMap.get(d);
    ...
    if(!route){results.push({transaction_id:tx.id,status:"no_curated_code_route"});continue}
    const label=clean(route.provider_label,180)||`${clean(tx.municipality,140)||"Municipality"} Code Enforcement`;
    const semantics="Watchdog identified the official municipal Code Enforcement or Property Maintenance office, but no parcel-specific violation search has been completed. Contact the office or use an authorized municipal process to verify open violations. The existence of this route cannot support a clean/no-violation conclusion.";
    const payload={...safe(item.payload),provider_family:"municipal_code_enforcement",official_code_enforcement_route:true,provider_registry_status:route.adapter_status,provider_access_mode:route.access_mode,route_verified_at:route.last_verified_at||null,route_metadata:safe(route.metadata),search_state:"not_run",search_completed:false,can_report_none:false,automation_authorized:false,manual_verification_required:true,block:clean(tx.block,60)||null,lot:clean(tx.lot,60)||null,qualifier:clean(tx.qualifier,60)||null,result_semantics:semantics};
    const updated=await admin.from("transaction_items").update({evidence_state:"verify",severity:"review",source_type:"official_manual",source_label:label,source_url:route.provider_url,source_checked_at:null,description:`Official municipal code-enforcement source identified for ${clean(tx.municipality,140)||"this municipality"}. ${semantics}`,payload,updated_at:routedAt}).eq("id",item.id).eq("user_id",user.id).select("id").single();
    if(updated.error){results.push({transaction_id:tx.id,status:"update_failed"});continue}
    const observation={district:d,municipality:tx.municipality||route.jurisdiction_name,provider_url:route.provider_url,search_state:"not_run",manual_verification_required:true,automation_authorized:false};
    await admin.from("transaction_evidence_observations").upsert({transaction_id:tx.id,user_id:user.id,pams_pin:tx.pams_pin,evidence_key:"code_violation_official_route",provider_key:`municipal-code-${d}`,evidence_status:"verify",value:observation,source_label:label,source_url:route.provider_url,source_checked_at:route.last_verified_at||routedAt,facts_hash:await digest(observation),metadata:{routing_only:true,search_completed:false,can_report_none:false,automation_authorized:false}},{onConflict:"transaction_id,evidence_key,provider_key,facts_hash",ignoreDuplicates:true});
    results.push({transaction_id:tx.id,status:"manual_verification_required",district:d,provider_url:route.provider_url});
  }
