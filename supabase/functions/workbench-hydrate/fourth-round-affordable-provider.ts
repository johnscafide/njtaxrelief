const DATA_URL='https://www.watchdogindex.com/property/data/fourth-round-affordable-published.json';
...
async function loadData(){
  ...
  try{
    ...
    if(!j?.validation?.publishable||Number(j?.validation?.matched_municipalities)!==564||!j?.municipalities)return null;
  ...
  for(const pin of pins){
    ...
    for(const id of ids){
      ...
      if(!root){
        ...
        payload.meta[pin][id]={status:'provider_error',provider_kind:'authoritative_reference',source:SOURCE,scope:'municipality',provider_version:VERSION,reason:'Governed Fourth Round published snapshot failed its 564-municipality validation gate or could not be loaded.'};
        continue;
      }
      if(!rec){
        delete payload.markers[pin][id];
