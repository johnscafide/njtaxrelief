...
Deno.serve(async(req:Request)=>{
 ...
 const [funnel,tools,acquisition,retention,intelligenceFunnel,intelligenceInteractions,todayActions,intentEvents,subscriptions,billingEvents,totalEvents]=await Promise.all([
  ...
  admin.from("intelligence_today_events").select("action,created_at").gte("created_at",start30).order("created_at",{ascending:false}).limit(5000),
  admin.from("intelligence_intent_events").select("event_type,fact_class,created_at").gte("created_at",start30).order("created_at",{ascending:false}).limit(5000),
 ...
 for(const result of [funnel,tools,acquisition,retention,intelligenceFunnel,intelligenceInteractions,todayActions,intentEvents,subscriptions,billingEvents,totalEvents])if(result.error)return json(req,500,{error:"Could not load one or more analytics aggregates"});
 const active=subscriptions.data||[],billing=billingEvents.data||[],today=todayActions.data||[],intent=intentEvents.data||[],latestDay=(funnel.data||[])[0]||null;
 return json(req,200,{ok:true,generated_at:new Date().toISOString(),privacy:"aggregate_product_analytics_only",summary:{total_product_events:totalEvents.count||0,latest_day:latestDay,active_subscription_count:active.length,active_by_plan:countBy(active,"plan_tier"),active_by_provider:countBy(active,"provider"),billing_lifecycle_events_30d:billing.length,billing_events_by_type_30d:countBy(billing,"event_type"),today_actions_30d:today.length,today_actions_by_type_30d:countBy(today,"action"),intent_events_30d:intent.length,intent_events_by_type_30d:countBy(intent,"event_type"),intent_events_by_fact_class_30d:countBy(intent,"fact_class")},funnel:funnel.data||[],tool_usage:tools.data||[],acquisition:acquisition.data||[],retention:retention.data||[],intelligence_funnel:intelligenceFunnel.data||[],intelligence_interactions:intelligenceInteractions.data||[]});
});