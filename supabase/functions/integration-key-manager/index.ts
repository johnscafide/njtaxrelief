...
function scopes(v:unknown){const xs=Array.isArray(v)?v.map(x=>clean(x,60)).filter(x=>ALLOWED_SCOPES.has(x)):DEFAULT_SCOPES.slice();if(!xs.includes("zapier.auth"))xs.unshift("zapier.auth");return [...new Set(xs)];}
...
Deno.serve(async(req)=>{
  ...
  if(action==="keys.list"){
    ...
    if(error)return reply(req,503,{error:"Integration keys could not be loaded"});return reply(req,200,{keys:data||[],plan,allowed_scopes:[...ALLOWED_SCOPES]});
  }
  if(action==="key.create"){
    ...
    const label=clean(body.label||"Zapier",80)||"Zapier",requestedScopes=scopes(body.scopes),token=`wdg_zap_${randomToken(32)}`,prefix=token.slice(0,16),hash=await sha256(token);
    const saved=await admin.from("integration_api_keys").insert({user_id:user.id,provider:"zapier",label,key_prefix:prefix,key_hash:hash,scopes:requestedScopes,status:"active"}).select("id,provider,label,key_prefix,scopes,status,created_at").single();
    if(saved.error)return reply(req,503,{error:"Zapier key could not be created"});await audit("zapier.key.created",{key_id:saved.data.id,label,scopes:requestedScopes});
    return reply(req,201,{key:saved.data,credential:{api_key:token,shown_once:true}});
  }
  if(action==="key.update"){
    const id=clean(body.key_id,80);if(!/^[0-9a-f-]{36}$/i.test(id))return reply(req,400,{error:"Valid key_id required"});const {data:key}=await admin.from("integration_api_keys").select("id,status,label,scopes").eq("id",id).eq("user_id",user.id).eq("provider","zapier").maybeSingle();if(!key)return reply(req,404,{error:"Zapier key not found"});if(key.status!=="active")return reply(req,409,{error:"Revoked keys cannot be edited"});const patch:Obj={};if(body.label!==undefined)patch.label=clean(body.label,80)||key.label;if(body.scopes!==undefined)patch.scopes=scopes(body.scopes);if(!Object.keys(patch).length)return reply(req,200,{key});const saved=await admin.from("integration_api_keys").update(patch).eq("id",id).eq("user_id",user.id).select("id,provider,label,key_prefix,scopes,status,last_used_at,created_at,revoked_at").single();if(saved.error)return reply(req,503,{error:"Zapier key could not be updated"});await audit("zapier.key.updated",{key_id:id,fields:Object.keys(patch)});return reply(req,200,{key:saved.data});
  }
