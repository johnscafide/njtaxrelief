...
Deno.serve(async(req:Request)=>{
  ...
  const normalizationManifest=safeObj(normalized?.normalization_manifest,50000),cohortManifest=safeObj(normalized?.cohort_manifest,30000),batchPayload={model_key:model.model_key,model_version:model.version,source_kind:"property_change_events",source_manifest:sourceManifest,normalization_manifest:normalizationManifest,cohort_manifest:cohortManifest,candidates:normalizedCandidates},batchHash=await hash(batchPayload);
  const {data:batch,error:bi}=await admin.from("intelligence_evidence_batches").insert({user_id:user.id,model_key:model.model_key,model_version:model.version,source_kind:"property_change_events",source_manifest:sourceManifest,normalization_manifest:normalizationManifest,cohort_manifest:cohortManifest,candidates:normalizedCandidates,facts_hash:batchHash,candidate_count:normalizedCandidates.length}).select("id").single();if(bi||!batch?.id)return out(req,503,{error:"Could not seal trusted change evidence batch"});
