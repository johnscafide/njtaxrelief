...
async function syncSubscription(sub: Stripe.Subscription, eventCreated?: number) {
  ...
  if (
    ...
    new Date(existing.provider_event_at).getTime() > new Date(eventAt).getTime()
  ) {
...
async function finalizeCredit(
  ...
  if (!id) return null;
  const r = await service.rpc('marketing_credit_finalize', {
    p_reservation_id: id,
    p_action: action,
    p_detail: detail
  });
...
async function recordCampaignPayment(session: Stripe.Checkout.Session, statusOverride?: string) {
  ...
  if (!meta?.watchdog_user_id || !meta?.campaign_id || !meta?.quote_id) {
    return { skipped: 'not marketing campaign checkout' };
  }
  ...
  const existing = await service
    ...
    .eq('user_id', meta.watchdog_user_id)
  ...
  const merged = {
    ...
    credit_applied_cents: Number(meta.credit_applied_cents || 0),
    retail_cents: Number(meta.retail_cents || existing.data?.amount_cents || 0),
    amount_due_cents: Number(meta.amount_due_cents || 0)
  };
  ...
  await service.from('marketing_campaigns').update({ status: campaignStatus, updated_at: new Date().toISOString() })
    .eq('id', meta.campaign_id)
    .eq('user_id', meta.watchdog_user_id);
  await service.from('marketing_events').insert({
    user_id: meta.watchdog_user_id,
...
async function expireCampaignPayment(session: Stripe.Checkout.Session) {
  ...
  await service.from('marketing_campaigns').update({ status: 'draft', updated_at: new Date().toISOString() })
    .eq('id', meta.campaign_id)
    .eq('user_id', meta.watchdog_user_id);
  await service.from('marketing_events').insert({
    user_id: meta.watchdog_user_id,
...
async function recordSubscriptionBillingSignal(
  eventType: string,
  ...
  const { data: entitlement, error } = await service
    .from('account_entitlements')
    .select('user_id,provider_subscription_id,billing_tier')
    .eq('provider', 'stripe')
    .eq('provider_customer_id', customerId)
    .maybeSingle();
  if (error) throw error;
...
async function recordRefund(charge: Stripe.Charge) {
  ...
  if (pi) {
    ...
    if (payment) {
      ...
      await service.from('marketing_campaigns').update({ status: status === 'refunded' ? 'refunded' : 'funded', updated_at: new Date().toISOString() })
      ...
      await service.from('marketing_events').insert({
        ...
        payload: {
          ...
          status,
          credit_applied_cents: Number(payment.metadata?.credit_applied_cents || 0)
        }
      });
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    const { data, error } = await service
      ...
      .insert({
        ...
        result: { status: 'processing', claimed_at: claimedAt }
      })
    ...
    const { data: prior, error: priorError } = await service
      .from('billing_webhook_events')
      .select('stripe_event_id,processed_at,result')
    ...
    if (processing && Number.isFinite(ageMs) && ageMs > 5 * 60 * 1000 && attempt === 0) {
      const { error: staleDeleteError } = await service
        ...
        .delete()
        .eq('stripe_event_id', event.id)
        .contains('result', { status: 'processing' });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error('STRIPE_WEBHOOK_CLAIM_ERROR', event.id, event.type, error);
    return new Response('Internal Server Error', { status: 500 });
  }

  if (!claim.claimed) {
    return new Response(claim.processing ? 'Processing' : 'Already processed', {
      status: claim.processing ? 202 : 200
    });
  }
  ...
  try {
    ...
    if (event.type.startsWith('customer.subscription.')) {
      ...
      if (campaignMeta(session)) {
        result = await recordCampaignPayment(session);
      ...
      result = await recordSubscriptionBillingSignal('billing.invoice_payment_failed', invoice.customer, {
      ...
      const invoice = event.data.object as Stripe.Invoice;
      result = await recordSubscriptionBillingSignal('billing.invoice_paid', invoice.customer, {
        invoice_id: invoice.id,
