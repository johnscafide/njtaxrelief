...
const SCORE_FILTER_MARKERS=[SCORE_ID,"watchdog.chapter123_annual_overpayment"];
...
function passFilters(row:any,f:Record<string,unknown>){
  ...
  if(active(f.wdMin)&&Number(row.__markers?.[SCORE_ID]??0)<Number(f.wdMin))return false;
...
Deno.serve(async(req:Request)=>{
  ...
  if(String(view.source_type)!=="saved"){
    ...
    const payload=await rr.json().catch(()=>({}));return out(req,rr.status,{...payload,view_adapter_engine:ENGINE,score_contract:{marker_id:SCORE_ID,model_version:SCORE_MODEL,framework:"ROBUST"}});
  }
  ...
  if(pins.length){for(let i=0;i<pins.length;i+=400){const {data:obs,error:oe}=await admin.from("score_observations").select("pams_pin,marker_id,score,observed_at,model_version").in("pams_pin",pins.slice(i,i+400)).in("marker_id",SCORE_FILTER_MARKERS).order("observed_at",{ascending:false});if(oe)return out(req,503,{error:"Saved-view marker filters could not be resolved"});const seen=new Set<string>();for(const o of obs||[]){if(String(o.marker_id)===SCORE_ID&&String(o.model_version)!==SCORE_MODEL)continue;const k=`${o.pams_pin}|${o.marker_id}`;if(seen.has(k))continue;seen.add(k);const row=sourceRows.find((x:any)=>String(x.pams_pin)===String(o.pams_pin));if(row)row.__markers[String(o.marker_id)]=numeric(o.score)}}}
  ...
  const filterManifest={semantics_version:"data-workbench-pass-filters-v2-robust",source_type:"saved",source_count:sourceRows.length,filtered_count:filteredPins.length,filters,filters_hash:await hash(filters),stable_filters:keys,score_contract:{marker_id:SCORE_ID,model_version:SCORE_MODEL,framework:"ROBUST"}};
  ...
  return out(req,rr.status,{...payload,view_adapter_engine:ENGINE,resolved_scope:{type:"workbench_view",id:view.id,name:view.name,source_type:view.source_type,filters,filtered_count:filteredPins.length},filter_manifest:filterManifest,score_contract:{marker_id:SCORE_ID,model_version:SCORE_MODEL,framework:"ROBUST"}});
});
