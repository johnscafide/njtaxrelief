...
const ENGINE_VERSION = 'watchdog-derived-v11-subject-index';
...
Deno.serve(async (req: Request) => {
  ...
  const needsChapter123 = operations.some((op) => ['revaluation_pressure','tax_reset_sensitivity','assessment_defensibility','appeal_evidence_strength','appeal_opportunity'].includes(op));
  ...
  for (const pin of pins) {
    ...
    const evalId = (id: string): any => {
      ...
      else if (def.operation === 'tax_rate_position') { const rateDefs: any[] = cfg.rate_deps || []; const points = rateDefs.map((x: any) => ({ year: Number(x.year), rate: num(value(x.dep)) })).filter((x: any) => Number.isFinite(x.year) && x.rate != null && x.rate > 0).sort((a: any, b: any) => a.year - b.year); if (points.length >= 3) { const first = points[0], last = points[points.length - 1], span = last.year - first.year; if (span >= 2) { const cagr = Math.pow(last.rate / first.rate, 1 / span) - 1; v = Math.round(100 - (positivePct5(cagr) ?? 100)); } } }
    ...
    for (const id of requested) {
      ...
      if (value !== null && value !== undefined && !Number.isNaN(value)) {
        const operation = String(def.operation || ''), chapterBacked = ['revaluation_pressure','tax_reset_sensitivity','assessment_defensibility','appeal_evidence_strength','appeal_opportunity'].includes(operation), directReference = operation === 'sr1a_subject_square_feet';
        markers[pin][id] = value;
        meta[pin][id] = { status: 'available', provider_kind: directReference ? 'authoritative_reference' : 'derived_governed', source: directReference ? 'NJ Division of Taxation SR-1A compact subject evidence index' : 'Watchdog governed formula registry · ' + ENGINE_VERSION, engine_version: ENGINE_VERSION, formula: def.formula, dependencies: def.dependencies, confidence: def.confidence, explanation: def.explanation, observed_at: now, ...(chapterBacked ? { reference_source: 'NJ Division of Taxation 2026 Chapter 123 · ' + CHAPTER123_PROVIDER } : {}), ...(directReference ? { subject_provider: SR1A_SUBJECT_PROVIDER } : {}), ...(auxMeta.get(id) || {}) };
      } else meta[pin][id] = { status: 'dependency_missing', provider_kind: String(def.operation || '') === 'sr1a_subject_square_feet' ? 'authoritative_reference' : 'derived_governed', source: String(def.operation || '') === 'sr1a_subject_square_feet' ? 'NJ Division of Taxation SR-1A compact subject evidence index' : 'Watchdog governed formula registry · ' + ENGINE_VERSION, engine_version: ENGINE_VERSION, dependencies: def.dependencies, checked_at: now, ...(String(def.operation || '') === 'sr1a_subject_square_feet' ? { subject_provider: SR1A_SUBJECT_PROVIDER } : {}), ...(auxMeta.get(id) || {}) };
    }
  }
