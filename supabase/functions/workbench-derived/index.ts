...
Deno.serve(async (req: Request) => {
  ...
  // For operations that intentionally consume their certified provider value, request those
  // dependencies from Hydrate explicitly instead of recursively re-evaluating them here.
  for (const id of seen) {
    const def:any=defMap.get(id),op=String(def?.operation||''),cfg=def?.config||{};
    if(op==='parcel_record_volatility'){
      if(cfg.change_dep) rawDeps.add(String(cfg.change_dep));
      if(cfg.history_dep) rawDeps.add(String(cfg.history_dep));
    }
  }
  const operations = [...seen].map((id) => String((defMap.get(id) as any)?.operation || ''));
  const needsChapter123 = operations.some((op) => ['revaluation_pressure','tax_reset_sensitivity','assessment_defensibility','appeal_evidence_strength','appeal_opportunity','chapter123_field','chapter123_position'].includes(op));
  const needsEvidenceReferences = operations.some((op) => ['sr1a_subject_square_feet','comparable_evidence_reliability','assessment_defensibility','appeal_evidence_strength','appeal_opportunity','chapter123_position'].includes(op));
  ...
  for (const pin of pins) {
    ...
    const evalId = (id: string): any => {
      ...
      else if (def.operation === 'assessment_component_shift') { const trace = orderedAssessmentHistory(value(cfg.land_dep), value(cfg.improvement_dep), null); if (trace) { let pair:any=null; for (let i=trace.length-1;i>0;i--) { const prev=trace[i-1],cur=trace[i],pl=num(prev.land),pi=num(prev.improvement),cl=num(cur.land),ci=num(cur.improvement); if (cur.year===prev.year+1&&pl!=null&&pi!=null&&cl!=null&&ci!=null&&pl>=0&&pi>=0&&cl>=0&&ci>=0&&(pl+pi)>0&&(cl+ci)>0) { pair={prev,cur,pl,pi,cl,ci}; break; } } if (pair) { const prevLand=pair.pl/(pair.pl+pair.pi)*100,prevImprovement=pair.pi/(pair.pl+pair.pi)*100,curLand=pair.cl/(pair.cl+pair.ci)*100,curImprovement=pair.ci/(pair.cl+pair.ci)*100,landDelta=curLand-prevLand,improvementDelta=curImprovement-prevImprovement,differential=landDelta-improvementDelta; v=round(clamp(50+differential/4),1); auxMeta.set(id,{from_year:pair.prev.year,to_year:pair.cur.year,previous_land_share_pct:round(prevLand,3),current_land_share_pct:round(curLand,3),previous_improvement_share_pct:round(prevImprovement,3),current_improvement_share_pct:round(curImprovement,3),land_share_delta_pp:round(landDelta,3),improvement_share_delta_pp:round(improvementDelta,3),component_share_differential_pp:round(differential,3),neutral_score:50,shift_direction:v>50?'toward_land':v<50?'toward_improvements':'unchanged',missing_years_synthesized:false,transition_contract:'latest consecutive published MOD-IV years with both components present'}); } } }
      else if (def.operation === 'parcel_record_volatility') { const changes=num(rawValue(String(cfg.change_dep||''))),history=yearObject(rawValue(String(cfg.history_dep||''))); if(changes!=null&&changes>=0&&history){ const years=Object.keys(history).filter((y)=>/^(19|20)\d{2}$/.test(y)).map(Number).filter(Number.isFinite).sort((a,b)=>a-b); let compared=0; for(let i=1;i<years.length;i++) if(years[i]===years[i-1]+1) compared++; if(compared>0&&changes<=compared){ v=round(clamp(changes/compared*100),1); auxMeta.set(id,{changed_consecutive_transitions:changes,compared_consecutive_transitions:compared,history_years:years,volatility_contract:'percent of actual consecutive annual MOD-IV transitions with any retained record-field change',missing_years_synthesized:false}); } } }
      else if (def.operation === 'source_release_freshness') { const gate=value(cfg.gate_dep),release=Date.parse(String(cfg.release_date||'')),cadence=num(cfg.cadence_days); if(present(gate)&&Number.isFinite(release)&&cadence!=null&&cadence>0){ const nowDate=new Date(),today=Date.UTC(nowDate.getUTCFullYear(),nowDate.getUTCMonth(),nowDate.getUTCDate()),rd=new Date(release),releaseDay=Date.UTC(rd.getUTCFullYear(),rd.getUTCMonth(),rd.getUTCDate()),ageDays=Math.max(0,Math.floor((today-releaseDay)/86400000)); v=round(clamp(100-ageDays/cadence*100),1); auxMeta.set(id,{source_release_date:String(cfg.release_date),source_age_days:ageDays,declared_refresh_cadence_days:cadence,freshness_contract:'100 on governed source release date; declines linearly to 0 at the declared refresh-cadence horizon',gate_dependency:String(cfg.gate_dep||'')}); } }
