...
const ENGINE_VERSION = 'watchdog-derived-v14-source-alias';
  ...
  for (const pin of pins) {
    ...
    for (const id of requested) {
      ...
      if (value !== null && value !== undefined && !Number.isNaN(value)) {
        const operation = String(def.operation || ''), chapterBacked = ['revaluation_pressure','tax_reset_sensitivity','assessment_defensibility','appeal_evidence_strength','appeal_opportunity'].includes(operation), sr1aDirect = operation === 'sr1a_subject_square_feet', sourceAlias = operation === 'source_alias', directReference = sr1aDirect || sourceAlias, aliasInfo: any = auxMeta.get(id) || {};
        markers[pin][id] = value;
        meta[pin][id] = { status: 'available', provider_kind: sourceAlias ? (aliasInfo.reference_provider_kind || 'authoritative_reference') : (directReference ? 'authoritative_reference' : 'derived_governed'), source: sourceAlias ? (aliasInfo.reference_source || 'Watchdog authoritative source alias') : (sr1aDirect ? 'NJ Division of Taxation SR-1A compact subject evidence index' : 'Watchdog governed formula registry · ' + ENGINE_VERSION), engine_version: ENGINE_VERSION, formula: def.formula, dependencies: def.dependencies, confidence: def.confidence, explanation: def.explanation, observed_at: sourceAlias ? (aliasInfo.reference_observed_at || now) : now, checked_at: sourceAlias ? (aliasInfo.reference_checked_at || null) : null, ...(chapterBacked ? { reference_source: 'NJ Division of Taxation 2026 Chapter 123 · ' + CHAPTER123_PROVIDER } : {}), ...(sr1aDirect ? { subject_provider: SR1A_SUBJECT_PROVIDER } : {}), ...(auxMeta.get(id) || {}) };
      } else {
        const operation = String(def.operation || ''), sourceAlias = operation === 'source_alias', aliasDep = String(def.config?.dep || ''), aliasMeta: any = aliasDep ? (rawMeta[aliasDep] || {}) : {};
        meta[pin][id] = { status: 'dependency_missing', provider_kind: operation === 'sr1a_subject_square_feet' ? 'authoritative_reference' : (sourceAlias ? (aliasMeta.provider_kind || 'authoritative_reference') : 'derived_governed'), source: operation === 'sr1a_subject_square_feet' ? 'NJ Division of Taxation SR-1A compact subject evidence index' : (sourceAlias ? (aliasMeta.source || 'Watchdog authoritative source alias') : 'Watchdog governed formula registry · ' + ENGINE_VERSION), engine_version: ENGINE_VERSION, dependencies: def.dependencies, checked_at: now, ...(operation === 'sr1a_subject_square_feet' ? { subject_provider: SR1A_SUBJECT_PROVIDER } : {}), ...(sourceAlias ? { alias_dependency: aliasDep } : {}), ...(auxMeta.get(id) || {}) };
      }
    }
  }
