...
const ENGINE_VERSION='watchdog-derived-v7-revaluation';
...
function cors(req:Request){
 ...
 return{
  'Access-Control-Allow-Origin':ORIGINS.has(o)?o:'https://njpropertytaxrelief.com',
...
Deno.serve(async req=>{
 ...
 const uc=createClient(url,anon,{global:{headers:{Authorization:auth}},auth:{persistSession:false}}),admin=createClient(url,service,{auth:{persistSession:false,autoRefreshToken:false}}),{data:who}=await uc.auth.getUser();
 if(!who.user)return out(req,401,{error:'Session invalid'});
 const {data:pd}=await admin.rpc('watchdog_effective_plan',{p_user_id:who.user.id}),plan=String(pd||'standard');
 ...
 const {data:defs,error:defErr}=await admin.from('derived_formula_registry').select('marker_id,formula,dependencies,confidence,status,explanation,operation,config').eq('status','live');
 if(defErr)return out(req,503,{error:'Formula registry unavailable'});
 const defMap=new Map((defs||[]).map((d:any)=>[String(d.marker_id),d])),requested=rawIds.filter(id=>defMap.has(id));
 ...
 const walk=(id:string)=>{if(seen.has(id))return;seen.add(id);const d:any=defMap.get(id);for(const dep of d?.dependencies||[]){if(defMap.has(dep))walk(dep);else rawDeps.add(dep)}};
 requested.forEach(walk);
 const needsChapter123=[...seen].some(id=>(defMap.get(id) as any)?.operation==='revaluation_pressure');
 ...
 const [hr,cr]=await Promise.all([hydratePromise,chapterPromise]);
 ...
 if(needsChapter123){
  if(!cr||!cr.ok)return out(req,503,{error:'Chapter 123 provider unavailable',status:cr?.status||503});
  const cj=await cr.json();
  if(cj?.source_id!=='nj-chapter123-2026'||Number(cj?.district_count)!==564)return out(req,503,{error:'Chapter 123 provider validation failed'});
  chapterDistricts=cj?.districts||{};
 }
 ...
 for(const pin of pins){
  ...
  const row=rowMap.get(pin)||{pams_pin:pin},rawVals:any=hydrated.markers?.[pin]||{},rawMeta:any=hydrated.meta?.[pin]||{},memo=new Map<string,any>(),stack=new Set<string>();
  ...
  const evalId=(id:string):any=>{
   if(memo.has(id))return memo.get(id);const d:any=defMap.get(id);if(!d||stack.has(id))return null;stack.add(id);
   const value=(dep:string)=>defMap.has(dep)?evalId(dep):rawValue(dep),cfg=d.config||{};let v:any=null;
   ...
   else if(d.operation==='revaluation_pressure'){
    const official=chapterDistricts[districtCode(pin)]||null,verified=num(value(cfg.verified_ratio_dep)),coeff=num(value(cfg.uniformity_dep));
    if(official&&verified!=null&&verified>0){
     const pub=Number(official.ratio)/100;
     if(pub>0){
      const level=clamp((0.85-pub)/0.35,0,1),decay=clamp((pub-verified)/0.20,0,1);let weighted=level*0.45+decay*0.25,w=0.70;
      if(coeff!=null&&coeff>0){weighted+=clamp((coeff-15)/20,0,1)*0.30;w+=0.30}
      let score=Math.round(weighted/w*100);if(pub>=0.98)score=Math.min(score,8);v=clamp(score);
     }
    }
   }
   else if(d.operation==='transaction_tax_shock'){
    const reval=num(value(cfg.reval_dep)),pressure=num(value(cfg.pressure_dep)),stability=num(value(cfg.tax_stability_dep)),levy=num(value(cfg.levy_dep)),ratable=num(value(cfg.ratable_dep));
    if(reval!=null&&pressure!=null&&stability!=null&&levy!=null&&ratable!=null){const taxRisk=clamp(100-stability),gapRisk=positivePct5(Math.max(0,levy-ratable));if(gapRisk!=null)v=Math.round(clamp(reval)*0.35+clamp(pressure)*0.25+taxRisk*0.25+gapRisk*0.15)}
   }
   else if(d.operation==='investor_carry_volatility'){
    const pressure=num(value(cfg.pressure_dep)),reval=num(value(cfg.reval_dep)),stability=num(value(cfg.tax_stability_dep)),levy=num(value(cfg.levy_dep)),ratable=num(value(cfg.ratable_dep)),exempt=num(value(cfg.exempt_share_dep));
    if(pressure!=null&&reval!=null&&stability!=null&&levy!=null&&ratable!=null&&exempt!=null){const taxRisk=clamp(100-stability),gapRisk=positivePct5(Math.max(0,levy-ratable)),exemptRisk=clamp(exempt/0.35*100);if(gapRisk!=null)v=Math.round(clamp(pressure)*0.32+gapRisk*0.24+clamp(reval)*0.22+taxRisk*0.14+exemptRisk*0.08)}
   }
   stack.delete(id);memo.set(id,v);return v;
  };
  for(const id of requested){
   const d:any=defMap.get(id),v=evalId(id);
   if(v!==null&&v!==undefined&&!Number.isNaN(v)){
    markers[pin][id]=v;
    meta[pin][id]={status:'available',provider_kind:'derived_governed',source:'Watchdog governed formula registry · '+ENGINE_VERSION,engine_version:ENGINE_VERSION,formula:d.formula,dependencies:d.dependencies,confidence:d.confidence,explanation:d.explanation,observed_at:now,...(d.operation==='revaluation_pressure'?{reference_source:'NJ Division of Taxation 2026 Chapter 123 · '+CHAPTER123_PROVIDER}: {})};
   }else meta[pin][id]={status:'dependency_missing',provider_kind:'derived_governed',source:'Watchdog governed formula registry · '+ENGINE_VERSION,engine_version:ENGINE_VERSION,dependencies:d.dependencies,checked_at:now};
  }
 }
 if(pins.length===1)return out(req,200,{pams_pin:pins[0],values:markers[pins[0]],records:hydrated.records||[],markers,meta,engine_version:ENGINE_VERSION});
