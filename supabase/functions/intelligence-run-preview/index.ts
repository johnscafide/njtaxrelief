...
function hasFilters(v:unknown){return !!v&&typeof v==="object"&&!Array.isArray(v)&&Object.keys(v as Record<string,unknown>).length>0}
...
Deno.serve(async(req:Request)=>{
  ...
  const modelKey=clean(body?.model_key,100),scopeType=clean(body?.scope_type||"custom",40),scopeRef=clean(body?.scope_id||body?.scope_value?.id,80);
  let pins=[...new Set((Array.isArray(body?.pams_pins)?body.pams_pins:[]).map((x:unknown)=>clean(x,100)).filter(Boolean))].slice(0,100);
  let resolvedScopeValue=safeObj(body?.scope_value,12000);
  if(!modelKey)return out(req,400,{error:"model_key is required"});
  if(!SCOPES.has(scopeType))return out(req,400,{error:"Unsupported Intelligence scope"});
  ...
  if((scopeType==="farm"||scopeType==="workbench_view")&&(RANK[plan]??0)<RANK.pro_plus)return out(req,403,{error:"Pro+ plan required for population Intelligence scopes",minimum_plan:"pro_plus"});

  if(scopeType==="farm"){
    const {data:rows,error}=await admin.from("agent_farm_properties").select("pams_pin").eq("user_id",user.id).eq("relationship","farm").not("pams_pin","is",null).limit(250);
    if(error)return out(req,503,{error:"Farm scope could not be resolved"});
    pins=[...new Set((rows||[]).map((x:any)=>clean(x.pams_pin,100)).filter(Boolean))];
    resolvedScopeValue={...resolvedScopeValue,source:"agent_farm_properties",resolved_count:pins.length};
  }else if(scopeType==="workbench_view"){
    if(!scopeRef)return out(req,400,{error:"scope_id is required for a Workbench view"});
    const {data:view,error}=await admin.from("data_workbench_views").select("id,name,source_type,source_id,filters,sort_config,page_size").eq("id",scopeRef).eq("user_id",user.id).maybeSingle();
    if(error||!view)return out(req,404,{error:"Workbench view not found"});
    if(hasFilters(view.filters))return out(req,409,{error:"This Workbench view contains filters that are not yet supported by the server-side Intelligence filter adapter"});
    if(view.source_type==="farm"){
      const {data:rows,error:fe}=await admin.from("agent_farm_properties").select("pams_pin").eq("user_id",user.id).eq("relationship","farm").not("pams_pin","is",null).limit(250);if(fe)return out(req,503,{error:"Workbench farm source could not be resolved"});pins=[...new Set((rows||[]).map((x:any)=>clean(x.pams_pin,100)).filter(Boolean))];
    }else if(view.source_type==="saved"){
      const {data:rows,error:se}=await admin.from("saved_properties").select("pams_pin").eq("user_id",user.id).not("pams_pin","is",null).limit(250);if(se)return out(req,503,{error:"Workbench saved-property source could not be resolved"});pins=[...new Set((rows||[]).map((x:any)=>clean(x.pams_pin,100)).filter(Boolean))];
    }else return out(req,409,{error:`Workbench view source '${clean(view.source_type,60)}' is not yet supported by Intelligence`});
    resolvedScopeValue={id:view.id,name:view.name,source_type:view.source_type,source_id:view.source_id||null,filters:view.filters||{},sort_config:view.sort_config||{},page_size:view.page_size,resolved_count:pins.length};
  }
  if(!pins.length)return out(req,409,{error:"The requested Intelligence scope resolved to no governed properties"});
  ...
  const sourceKeys=[...new Set([...featureKeys.map(k=>defs.get(k)?.source_key),...[...defs.values()].map((d:any)=>d?.config?.guard_source_key)].filter(Boolean).map((x:unknown)=>clean(x,140)))];
  ...
  const records=Array.isArray(hydrated?.records)?hydrated.records:[],recordMap=new Map(records.map((r:any)=>[String(r.pams_pin),r]));if(!records.length)return out(req,409,{error:"No governed property records were resolved"});

  const cohortDefs=new Map<string,any>(),cohortKeys=[...new Set([...defs.values()].map((d:any)=>d.cohort_key).filter(Boolean))];
  if(cohortKeys.length){const {data:rows,error:ce}=await admin.from("intelligence_cohort_versions").select("cohort_key,version,dimensions,minimum_sample_size,fallback_chain,status").in("cohort_key",cohortKeys);if(ce)return out(req,503,{error:"Cohort registry unavailable"});for(const c of rows||[])cohortDefs.set(`${c.cohort_key}:${c.version}`,c)}
  const cohortCache=new Map<string,{values:number[];resolution:any}>();
  async function resolveCohort(featureKey:string,def:any,row:any){if(!def?.cohort_key)return{values:[],resolution:null};const cd=cohortDefs.get(`${def.cohort_key}:${def.cohort_version}`);if(!cd)return{values:[],resolution:{status:"definition_missing"}};const fallbacks=Array.isArray(cd.fallback_chain)?cd.fallback_chain:[],scopes=[String(def.cohort_key),...fallbacks.map((x:unknown)=>clean(x,80))],minimum=Number(cd.minimum_sample_size||12);for(const scope of scopes){const key=[featureKey,scope,row?.town||"",row?.county||"",row?.prop_class||""].join("|"),old=cohortCache.get(key);if(old&&old.values.length>=minimum)return old;let q=admin.from("property_lookups").select("pams_pin,town,county,prop_class,assessed_value,last_year_tax").limit(5000);if(scope==="municipality_property_class")q=q.eq("town",row.town).eq("prop_class",row.prop_class);else if(scope==="municipality")q=q.eq("town",row.town);else if(scope==="county_property_class")q=q.eq("county",row.county).eq("prop_class",row.prop_class);else if(scope==="county")q=q.eq("county",row.county);else continue;const {data:peers,error:qe}=await q;if(qe)continue;const pairs=(peers||[]).map((p:any)=>({pams_pin:String(p.pams_pin),value:rawFromProperty(String(def.source_key),p)})).filter((p:any)=>num(p.value)!==null).map((p:any)=>({pams_pin:p.pams_pin,value:Number(p.value)})),values=pairs.map((p:any)=>p.value),resolution={status:values.length>=minimum?"resolved":"insufficient",requested_cohort:def.cohort_key,resolved_scope:scope,sample_size:values.length,minimum_sample_size:minimum,population_hash:await hash(pairs.sort((a:any,b:any)=>a.pams_pin.localeCompare(b.pams_pin)))},result={values,resolution};cohortCache.set(key,result);if(values.length>=minimum)return result}return{values:[],resolution:{status:"insufficient",requested_cohort:def.cohort_key,minimum_sample_size:minimum}}}

  const normalizeCandidates:any[]=[],resolvedCohorts:Record<string,unknown>={};
  for(const pin of pins){const row=recordMap.get(pin);if(!row)continue;const raw:Record<string,unknown>={},source_meta:Record<string,unknown>={},cohorts:Record<string,unknown[]>={};for(const sk of sourceKeys){const value=hydrated?.markers?.[pin]?.[sk],m=hydrated?.meta?.[pin]?.[sk]||{};if(value!==undefined&&value!==null)raw[sk]=value;source_meta[sk]={source_key:sk,source_url:m?.source_url||null,observed_at:m?.observed_at||m?.checked_at||hydrated?.checked_at||null,explanation:m?.source||m?.explanation||null}}for(const fk of featureKeys){const def=defs.get(fk);if(!def)continue;if(def.cohort_key){const cr=await resolveCohort(fk,def,row);cohorts[fk]=cr.values;if(cr.resolution){const rk=[def.cohort_key,row?.town||"",row?.county||"",row?.prop_class||""].join("|");resolvedCohorts[rk]=cr.resolution}}}normalizeCandidates.push({pams_pin:pin,address:row?.address||null,raw,cohorts,source_meta})}
  if(!normalizeCandidates.length)return out(req,409,{error:"No candidates remained after governed hydration"});
  const nr=await fetch(`${url}/functions/v1/intelligence-normalize-preview`,{method:"POST",headers:{Authorization:auth,apikey:publishable,"Content-Type":"application/json"},body:JSON.stringify({candidates:normalizeCandidates,feature_keys:featureKeys})}),normalized=await nr.json().catch(()=>({}));if(!nr.ok)return out(req,nr.status,{error:"Intelligence normalization failed",detail:clean(normalized?.error,300)||null});
  ...
  const sr=await fetch(`${url}/functions/v1/intelligence-score-preview`,{method:"POST",headers:{Authorization:auth,apikey:publishable,"Content-Type":"application/json"},body:JSON.stringify({evidence_batch_id:batch.id,limit:body?.limit||50,requested_prompt:clean(body?.requested_prompt,1200)||null})}),scored=await sr.json().catch(()=>({}));if(!sr.ok)return out(req,sr.status,{error:"Trusted Intelligence scoring failed",detail:clean(scored?.error,300)||null,evidence_batch_id:batch.id});
  return out(req,200,{...scored,pipeline_engine:ENGINE,scope:{type:scopeType,value:resolvedScopeValue},hydration:{source:hydrated?.source||null,provider_summary:hydrated?.provider_summary||{},checked_at:hydrated?.checked_at||null},normalization:{engine_version:normalized?.engine_version||null,feature_count:normalized?.feature_count||0},cohorts:{resolved_count:Object.keys(resolvedCohorts).length}});
});
