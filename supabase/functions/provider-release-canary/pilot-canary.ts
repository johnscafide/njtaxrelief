...
export async function handlePilotCanary(req: Request) {
  let body: any = {};
  ...
  try {
    ...
    const acct = await admin.from('watchdog_test_accounts').upsert({
      user_id: userId, label: 'PILOT Provider Release Canary', last_bootstrap_at: now,
    ...
    const evidence = {
      scenario: SCENARIO, target_function: 'workbench-hydrate', status_code: response.status,
      duration_ms: Date.now() - started, assertion,
      provider_version: payload?.provider_versions?.dca_pilot_observed || null,
      plan: payload?.plan || null,
    };

    await admin.from('watchdog_test_auth_events').insert({
      token_id: gate.id, user_id: userId, event_type: 'provider_release_canary',
    ...
    return json(ok ? 200 : 502, { ok, ...evidence });
  } catch (error) {
