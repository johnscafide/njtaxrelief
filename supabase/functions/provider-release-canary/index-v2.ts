...
type Scenario={fn:string;body:any;expect_available?:AvailabilityExpectation[]};
const SCENARIOS:Record<string,Scenario>={
  ...
  csrr_v1:{fn:'workbench-hydrate',body:{pams_pins:['0505_824.02_12'],marker_ids:['njplus.njdep-csrr-gis.kcsl_case_status','njplus.njdep-csrr-gis.kcsl_program_interest','njplus.njdep-csrr-gis.kcsl_site_id','njplus.njdep-csrr-gis.environmental_layer_vintage']}},
  csrr_controls_v2:{fn:'workbench-hydrate',body:{pams_pins:['0102_830_10','0505_824.02_12'],marker_ids:['njplus.njdep-csrr-gis.deed_notice_reference','njplus.njdep-csrr-gis.cea_reference','njplus.njdep-csrr-gis.ust_facility_reference','njplus.njdep-csrr-gis.brownfield_inventory_status']},expect_available:[{pin:'0102_830_10',marker_ids:['njplus.njdep-csrr-gis.deed_notice_reference','njplus.njdep-csrr-gis.ust_facility_reference','njplus.njdep-csrr-gis.brownfield_inventory_status']},{pin:'0505_824.02_12',marker_ids:['njplus.njdep-csrr-gis.cea_reference']}]}
};
...
function availabilityAssertion(scenario:Scenario,payload:any){const expected=scenario.expect_available||[];if(!expected.length)return{ok:true,missing:[] as string[]};const missing:string[]=[];for(const check of expected){for(const id of check.marker_ids){if(String(payload?.meta?.[check.pin]?.[id]?.status||'')!=='available'||payload?.markers?.[check.pin]?.[id]===null||payload?.markers?.[check.pin]?.[id]===undefined)missing.push(`${check.pin}:${id}`)}}return{ok:missing.length===0,missing}}
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const authClient=createClient(URL,ANON,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false}});const {data:verified,error:verifyError}=await authClient.auth.verifyOtp({token_hash:hashed,type:'email'});const accessToken=verified?.session?.access_token||'';if(verifyError||!accessToken)throw new Error('sandbox_session_verification_failed');
    const profile=await admin.from('profiles').upsert({id:userId,email,full_name:'Watchdog Provider Release Canary',display_name:'Watchdog Provider Release Canary',account_role:'developer',plan_tier:'standard',plan:'free',profile_complete:true,custom:{watchdog_test_account:true,no_real_spend:true,release_canary:true}},{onConflict:'id'});if(profile.error)throw new Error('sandbox_profile_failed');
    const acct=await admin.from('watchdog_test_accounts').upsert({user_id:userId,label:'Provider Release Canary',last_bootstrap_at:now,metadata:{email,no_real_spend:true,scenario:scenarioKey}},{onConflict:'user_id'});if(acct.error)throw new Error('sandbox_account_failed');
    const started=Date.now();const response=await fetch(`${URL}/functions/v1/${scenario.fn}`,{method:'POST',headers:{Authorization:`Bearer ${accessToken}`,apikey:ANON,'Content-Type':'application/json'},body:JSON.stringify(scenario.body)});const text=await response.text();let payload:any=null;try{payload=JSON.parse(text)}catch{payload={raw:text.slice(0,500)}}
    const assertion=availabilityAssertion(scenario,payload),ok=response.ok&&assertion.ok;const evidence={scenario:scenarioKey,target_function:scenario.fn,status_code:response.status,duration_ms:Date.now()-started,assertion,payload};await admin.from('watchdog_test_auth_events').insert({token_id:gate.id,user_id:userId,event_type:'provider_release_canary',metadata:{scenario:scenarioKey,status_code:response.status,duration_ms:evidence.duration_ms,assertion_ok:assertion.ok,missing:assertion.missing}});return json(req,ok?200:502,{ok,...evidence});
  }catch(error){return json(req,500,{ok:false,scenario:scenarioKey,error:String((error as Error)?.message||error)})}finally{if(userId)await cleanup(userId)}
});
