...
Deno.serve(async(req)=>{
  ...
  const url=Deno.env.get("SUPABASE_URL")||"",secret=namedEnv("SUPABASE_SECRET_KEYS","SUPABASE_SERVICE_ROLE_KEY");
  if(!url||!secret)return reply(503,{error:"Integration worker unavailable"});
  const db=createClient(url,secret,{auth:{persistSession:false,autoRefreshToken:false}});
  const gotAutomation=req.headers.get("x-watchdog-automation-secret")||"",expectedAutomation=Deno.env.get("WATCHDOG_AUTOMATION_SECRET")||Deno.env.get("AGENT_DIGEST_CRON_SECRET")||"",gotWorker=req.headers.get("x-watchdog-worker-token")||"";
  let authorized=!!expectedAutomation&&same(gotAutomation,expectedAutomation);
  ...
  let body:Obj={};try{body=await req.json()}catch{}
  const requested=clean(body.delivery_id,80),now=new Date().toISOString();
  let q=db.from("integration_deliveries").select("id,event_id,connection_id,user_id,status,attempt_count,max_attempts,next_attempt_at,manual_replay_count").eq("status","pending").lte("next_attempt_at",now).order("next_attempt_at",{ascending:true}).limit(requested?1:25);
  if(requested)q=q.eq("id",requested);
  const due=await q;
  if(due.error)return reply(503,{error:"Pending deliveries could not be loaded"});

  let delivered=0,retried=0,failed=0,canceled=0,skipped=0;
  for(const d of due.data||[]){
    const attemptNo=Number(d.attempt_count||0)+1;
    const claim=await db.from("integration_deliveries").update({status:"processing",attempt_count:attemptNo,last_attempt_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",d.id).eq("status","pending").select("id").maybeSingle();
    if(claim.error||!claim.data){skipped++;continue;}

    const [cr,er,controlResult]=await Promise.all([
      db.from("integration_connections").select("id,user_id,name,provider,status,direction,outbound_url,outbound_secret_id,event_types").eq("id",d.connection_id).maybeSingle(),
      db.from("integration_events").select("id,event_type,event_key,source,status,payload,occurred_at").eq("id",d.event_id).maybeSingle(),
      db.from("integration_connection_controls").select("outbound_enabled,external_writes_enabled,disabled_event_types,reason").eq("connection_id",d.connection_id).eq("user_id",d.user_id).maybeSingle()
    ]);
    ...
    const providerControlResult=await db.from("integration_provider_controls").select("outbound_enabled,external_writes_enabled,disabled_event_types,reason").eq("user_id",d.user_id).eq("provider",c.provider).maybeSingle();
    const providerControl=providerControlResult.data;
    const providerDisabled=Array.isArray(providerControl?.disabled_event_types)?providerControl.disabled_event_types:[];
    if(providerControl&&(!providerControl.outbound_enabled||providerControl.external_writes_enabled===false||providerDisabled.includes(e.event_type))){
      const why=!providerControl.outbound_enabled?"provider_outbound_disabled":providerControl.external_writes_enabled===false?"provider_external_writes_disabled":"provider_event_type_disabled";
      const message=clean(providerControl.reason||"Provider automation disabled by Watchdog control plane",500);
      await db.from("integration_deliveries").update({status:"canceled",last_error:message,updated_at:new Date().toISOString()}).eq("id",d.id);
      await health(db,d.user_id,d.connection_id,`provider:${c.provider}`,"disabled",why,message,{provider:c.provider,event_type:e.event_type});
      canceled++;continue;
    }

    const disabledTypes=Array.isArray(control?.disabled_event_types)?control.disabled_event_types:[];
    if(control&&(!control.outbound_enabled||control.external_writes_enabled===false||disabledTypes.includes(e.event_type))){
      const why=!control.outbound_enabled?"outbound_disabled":control.external_writes_enabled===false?"external_writes_disabled":"event_type_disabled";
      const message=clean(control.reason||"Outbound delivery disabled by Watchdog control plane",500);
      await db.from("integration_deliveries").update({status:"canceled",last_error:message,updated_at:new Date().toISOString()}).eq("id",d.id);
      await health(db,d.user_id,d.connection_id,"connection","disabled",why,message,{event_type:e.event_type});
      canceled++;continue;
    }

    const destination=publicHttps(c.outbound_url);
    if(!destination||!c.outbound_secret_id){
    ...
    const sec=await db.rpc("integration_get_secret",{p_secret_id:c.outbound_secret_id});
    ...
    const payload={schema_version:"2026-08-19",delivery_id:d.id,event_id:e.id,event_type:e.event_type,event_key:e.event_key,occurred_at:e.occurred_at,source:e.source||"watchdog",data:e.payload||{}};
    const raw=JSON.stringify(payload),signature=await hmac(String(sec.data),`${timestamp}.${raw}`),started=Date.now();
    let ok=false,status:number|null=null,excerpt="",errorCode="";
    try{
      const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),8000);
      const res=await fetch(destination,{method:"POST",redirect:"manual",signal:controller.signal,headers:{"Content-Type":"application/json","User-Agent":"Watchdog-Integrations/1.0","X-Watchdog-Event":e.event_type,"X-Watchdog-Delivery":d.id,"X-Watchdog-Connection":c.id,"X-Watchdog-Timestamp":timestamp,"X-Watchdog-Signature":`v1=${signature}`,"Idempotency-Key":e.id},body:raw});
      clearTimeout(timer);status=res.status;excerpt=clean(await res.text(),500);ok=res.status>=200&&res.status<300;if(!ok)errorCode=`http_${res.status}`;
