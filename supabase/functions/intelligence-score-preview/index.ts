...
const ENGINE = "watchdog-intelligence-preview-v4-context-lineage";
...
const env = (jsonKey: string, legacy: string) => {
...
async function hash(v: unknown) {
  const d = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(JSON.stringify(canon(v))));
  return [...new Uint8Array(d)].map((x) => x.toString(16).padStart(2, "0")).join("");
}

function obj(v: unknown, max = 50000) {
  if (!v || typeof v !== "object" || Array.isArray(v)) return {};
  try { return JSON.stringify(v).length <= max ? v as Record<string, unknown> : {}; } catch { return {}; }
}

function available(f: any) {
  return !!f && (!f.status || f.status === "available") && num(f.score) !== null;
}

function prov(f: any) {
  return {
    normalization: f?.normalization && typeof f.normalization === "object" ? {
      feature_version: num(f.normalization.feature_version),
      transform_type: clean(f.normalization.transform_type, 100) || null,
      direction: clean(f.normalization.direction, 80) || null,
      detail: obj(f.normalization.detail, 4000),
    } : null,
    cohort: f?.cohort && typeof f.cohort === "object" ? obj(f.cohort, 6000) : null,
    lineage: f?.lineage && typeof f.lineage === "object" ? {
      status: clean(f.lineage.status, 80) || null,
      provider_kind: clean(f.lineage.provider_kind, 120) || null,
      engine_version: clean(f.lineage.engine_version, 160) || null,
      formula: clean(f.lineage.formula, 1000) || null,
      dependencies: Array.isArray(f.lineage.dependencies)
        ? f.lineage.dependencies.map((x: unknown) => clean(x, 140)).filter(Boolean).slice(0, 40)
        : [],
    } : null,
  };
}

function sourceFields(f: any) {
  return {
    source_key: clean(f?.source_key, 140) || null,
    source_url: clean(f?.source_url, 600) || null,
    observed_at: clean(f?.observed_at, 80) || null,
    explanation: clean(f?.explanation, 500) || null,
  };
}

function latestObserved(evidence: any[]) {
  let latest = 0;
  let value: string | null = null;
  for (const e of evidence) {
    const t = new Date(String(e?.observed_at || "")).getTime();
...
Deno.serve(async (req: Request) => {
  ...
  const uc = createClient(url, publishable, { global: { headers: { Authorization: auth } }, auth: { persistSession: false, autoRefreshToken: false } });
  const admin = createClient(url, secret, { auth: { persistSession: false, autoRefreshToken: false } });
  const { data: au } = await uc.auth.getUser();
  const user = au?.user;
  ...
  const batchId = clean(body?.evidence_batch_id, 80);
  if (!batchId) return out(req, 400, { error: "evidence_batch_id is required" });
  ...
  const [{ data: ent }, { data: profile }, { data: model, error: me }] = await Promise.all([
    ...
    admin.from("intelligence_model_versions")
      .select("model_key,label,objective,minimum_plan,version,status,calibration_state,signal_config")
  ...
  const signals = (Array.isArray(cfg.signals) ? cfg.signals : [])
    .map((s: any) => ({ id: clean(s?.id, 140), role: clean(s?.role || "score", 30), weight: Number(s?.weight ?? 0) }))
    .filter((s: any) => s.id);
  const scoredDefs = signals.filter((s: any) => s.role !== "confidence" && s.weight > 0);
  const confidenceDefs = signals.filter((s: any) => s.role === "confidence");
  if (!scoredDefs.length) return out(req, 409, { error: "Model has no deterministic score configuration" });

  const totalWeight = scoredDefs.reduce((a: number, s: any) => a + s.weight, 0);
  const minimumCoverage = clamp(Number(cfg.minimum_evidence_coverage ?? 0));
  const actions = Array.isArray(cfg.recommended_actions)
    ? cfg.recommended_actions.map((x: unknown) => clean(x, 80)).filter(Boolean)
    : ["review_evidence"];
  const sourceManifest = obj(batch.source_manifest);
  const normalizationManifest = obj(batch.normalization_manifest);
  const cohortManifest = obj(batch.cohort_manifest);
  const requestedScope = clean((sourceManifest as any).scope_type || body?.scope_type || "custom", 40);
  const scopeType = SCOPES.has(requestedScope) ? requestedScope : "custom";
  const scopeValue = obj((sourceManifest as any).scope_value || body?.scope_value, 20000);

  const { data: run, error: re } = await admin.from("intelligence_runs").insert({
    user_id: user.id,
    ...
    requested_prompt: clean(body?.requested_prompt, 1200) || null,
    status: "running",
  ...
  try {
    ...
    for (const c of candidates) {
      ...
      const propertyContext = obj(c?.context, 6000);
      let weighted = 0;
      ...
      for (const s of scoredDefs) {
        const f = features[s.id];
        const p = prov(f);
        if (!available(f)) {
          missing.push({ signal_id: s.id, role: s.role || "score", weight: s.weight, reason: clean(f?.status || "missing", 80), ...sourceFields(f), ...p });
          continue;
        }
        const fs = clamp(Number(f.score));
        const ct = fs * s.weight;
      ...
      for (const s of confidenceDefs) {
        ...
        const p = prov(f);
        if (!available(f)) {
          missing.push({ signal_id: s.id, role: "confidence", reason: clean(f?.status || "missing", 80), ...sourceFields(f), ...p });
          continue;
        }
        evidence.push({
          signal_id: s.id,
          role: "confidence",
          score: round(clamp(Number(f.score))),
          value: f?.value ?? null,
          ...sourceFields(f),
          ...p,
        });
      ...
      const cv = confidenceDefs.map((s: any) => features[s.id]).filter(available).map((f: any) => clamp(Number(f.score)));
      const cm = cv.length ? cv.reduce((a: number, b: number) => a + b, 0) / cv.length : null;
      let confidence = cm === null ? coverage : coverage * 0.7 + cm * 0.3;
      const limited = coverage < minimumCoverage;
      if (limited) confidence = Math.min(confidence, 49);
      confidence = clamp(confidence);

      const latestEvidenceAt = latestObserved(evidence);
      const fact = {
        model_key: model.model_key,
        model_version: model.version,
        evidence_batch_hash: batch.facts_hash,
        candidate: {
          pams_pin: clean(c?.pams_pin, 100) || null,
          address: clean(c?.address, 300) || null,
      ...
      const factsHash = await hash(fact);
      const whyNow = contrib
      ...
      scored.push({
        user_id: user.id,
        pams_pin: clean(c?.pams_pin, 100) || null,
        property_address: clean(c?.address, 300) || null,
    ...
    const findings = selected.map((x, i) => ({
      run_id: runId,
      user_id: x.user_id,
      pams_pin: x.pams_pin,
      property_address: x.property_address,
      opportunity_type: x.opportunity_type,
      rank: i + 1,
      score: x.score,
      confidence: x.confidence,
      evidence_coverage: x.evidence_coverage,
      potential_impact: x.potential_impact,
      why_now: x.why_now,
      evidence: x.evidence,
      missing_evidence: x.missing_evidence,
      recommended_actions: x.recommended_actions,
      narrative: x.narrative,
      narrative_status: x.narrative_status,
      facts_hash: x.facts_hash,
    }));

    if (findings.length) {
      const ins = await admin.from("intelligence_findings").insert(findings);
      if (ins.error) throw ins.error;
    }

    const runHash = await hash({
      model_key: model.model_key,
      model_version: model.version,
      evidence_batch_hash: batch.facts_hash,
      scope_type: scopeType,
      scope_value: scopeValue,
      candidate_hashes: scored.map((x) => x.facts_hash),
    });
    const done = new Date().toISOString();
    const [ru, bu] = await Promise.all([
      admin.from("intelligence_runs").update({ status: "complete", finding_count: findings.length, facts_hash: runHash, completed_at: done }).eq("id", runId),
      admin.from("intelligence_evidence_batches").update({ consumed_at: done }).eq("id", batch.id).is("consumed_at", null),
    ]);
    ...
    return out(req, 200, {
      ...
      findings: findings.map((f, i) => ({
        ...f,
        property_context: selected[i]?.property_context || {},
        latest_evidence_at: selected[i]?.latest_evidence_at || null,
        limited_evidence: selected[i]?.limited_evidence ?? false,
      })),
      warning: model.calibration_state === "calibrated" ? null : "Preview model: deterministic output is not yet calibrated or predictive.",
    });
  } catch (error) {
    await admin.from("intelligence_runs").update({ status: "failed", error_code: "preview_score_failed", completed_at: new Date().toISOString() }).eq("id", runId);
    console.error("intelligence-score-preview", error);
    return out(req, 500, { error: "Intelligence preview run failed", run_id: runId });
  }
});
