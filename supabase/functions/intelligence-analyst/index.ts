...
const featureActive=(row:O|null|undefined)=>{const status=clean(row?.status,40).toLowerCase();if(!["active","trialing"].includes(status))return false;const end=row?.current_period_end?Date.parse(String(row.current_period_end)):NaN;return !Number.isFinite(end)||end>Date.now()};
...
Deno.serve(async(req:Request)=>{
  ...
  const [{data:ent},{data:profile},{data:promptRows},{data:addon}]=await Promise.all([admin.from("account_entitlements").select("plan_tier,profession").eq("user_id",user.id).maybeSingle(),admin.from("profiles").select("account_role").eq("id",user.id).maybeSingle(),admin.from("intelligence_prompt_versions").select("prompt_key,version,model,system_contract,status").eq("prompt_key","watchdog_analyst").in("status",["preview","live"]).order("version",{ascending:false}).limit(1),admin.from("account_feature_entitlements").select("status,current_period_end").eq("user_id",user.id).eq("feature_key",INTELLIGENCE_ADDON_FEATURE).maybeSingle()]);
  const plan=String(profile?.account_role||"")==="developer"?"developer":String(ent?.plan_tier||"standard"),addonActive=plan==="agent"&&featureActive(addon),analystAllowed=(PLAN_RANK[plan]??0)>=PLAN_RANK.pro||addonActive;if(!analystAllowed)return out(req,403,{error:"Pro plan or active Watchdog Intelligence add-on required",minimum_plan:"pro",feature_key:INTELLIGENCE_ADDON_FEATURE});const profession=clean(ent?.profession||context.profession||"general",80)||"general";
  ...
  if(blocked){const response={conclusion:blocked,evidence:[],missing_evidence:[],caveats:["Watchdog did not run a property tool for this request."],suggested_actions:[],sources:[]};await admin.from("intelligence_analyst_messages").insert({session_id:sessionId,user_id:user.id,role:"assistant",content:response,prompt_key:promptRow.prompt_key,prompt_version:promptRow.version,tool_contract_version:TOOL_VERSION,status:"refused"});await admin.from("intelligence_usage_events").insert({user_id:user.id,plan_tier:plan,event_type:"analyst_request",metadata:{status:"refused",reason:"guardrail",analyst_version:ANALYST_VERSION,access_path:addonActive?"watchdog_intelligence_add_on":"plan"}});return out(req,200,{ok:true,session_id:sessionId,status:"refused",provider_status:"not_called",response})}
