...
Deno.serve(async (req) => {
  ...
  const { data: entitlement } = await admin.from('account_entitlements').select('provider,provider_customer_id,provider_subscription_id,subscription_status').eq('user_id', user.id).maybeSingle();
  ...
  try {
    transaction = await paddle('/transactions', {
      ...
      body: JSON.stringify({
        ...
        custom_data: { supabase_user_id: user.id, watchdog_requested_plan: plan },
        checkout: { url: 'https://njpropertytaxrelief.com/property/account.html' }
      })
    });
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : 'Could not create checkout' }, 502);
  }
  await admin.from('access_audit_log').insert({ user_id: user.id, event_type: 'billing.checkout_created', resource_type: 'transaction', resource_id: transaction?.id, required_plan: plan, allowed: true, metadata: { provider: 'paddle' } });
  return json({
    provider: 'paddle',
