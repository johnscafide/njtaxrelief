...
Deno.serve(async(req:Request)=>{
  ...
  const url=Deno.env.get("SUPABASE_URL")||"",anon=Deno.env.get("SUPABASE_ANON_KEY")||"",service=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")||"";if(!url||!anon||!service)return respond(req,503,{error:"Municipal requirements configuration incomplete"});
  ...
  const {data:txs,error:txErr}=await admin.from("transaction_workspaces").select("id,user_id,address,municipality,county,pams_pin").eq("user_id",user.id).in("id",ids);if(txErr)return respond(req,503,{error:"Transactions unavailable"});
  const districts=[...new Set((txs||[]).map((t:Row)=>district(t)).filter(Boolean))];const {data:reqRows,error:reqErr}=districts.length?await admin.from("transaction_municipal_requirements").select("*").in("municipality_code",districts):{data:[],error:null};if(reqErr)return respond(req,503,{error:"Municipal requirements registry unavailable"});
  const reqMap=new Map<string,Row>();for(const r of reqRows||[])reqMap.set(`${r.municipality_code}:${r.requirement_key}`,r);
  const itemKeys=["resale_cco","smoke_fire_cert"];const {data:items}=await admin.from("transaction_items").select("id,transaction_id,item_key,payload").eq("user_id",user.id).in("transaction_id",ids).in("item_key",itemKeys);const itemMap=new Map<string,Row>();for(const i of items||[])itemMap.set(`${i.transaction_id}:${i.item_key}`,i);
  const checkedAt=new Date().toISOString(),results:Row[]=[];
  for(const tx of txs||[]){const code=district(tx);if(!code){results.push({transaction_id:tx.id,status:"municipality_code_missing"});continue}const txResult:Row={transaction_id:tx.id,municipality_code:code,requirements:{}};
    for(const key of itemKeys){const r=reqMap.get(`${code}:${key}`),item=itemMap.get(`${tx.id}:${key}`);if(!r||!item){txResult.requirements[key]="registry_or_item_missing";continue}
      const requirements=Array.isArray(r.requirements)?r.requirements:[],fees=Array.isArray(r.fees)?r.fees:[],sources=Array.isArray(r.source_urls)?r.source_urls:[];const explicit=r.requirement_state==="explicit_required",process=r.requirement_state==="official_process_found",baseline=r.requirement_state==="statewide_baseline";
      const sourceUrl=r.application_url||r.department_url||r.ordinance_url||sources?.[0]?.url||null;let description="";
      if(key==="resale_cco"){
      ...
      await admin.from("transaction_items").update({title:r.title,evidence_state:"verify",severity:explicit?"attention":"review",source_type:"official_requirement",source_label:`${r.municipality_name} official sources`,source_url:sourceUrl,source_checked_at:r.last_verified_at||checkedAt,description,payload,updated_at:checkedAt}).eq("id",item.id).eq("user_id",user.id);
      const evidence={requirement_key:key,requirement_state:r.requirement_state,requirements,fees,application_url:r.application_url,department_url:r.department_url,ordinance_url:r.ordinance_url,sources};await admin.from("transaction_evidence_observations").upsert({transaction_id:tx.id,user_id:user.id,pams_pin:tx.pams_pin,evidence_key:`municipal_requirement_${key}`,provider_key:`municipal-requirements-${code}`,evidence_status:"verify",value:evidence,source_label:`${r.municipality_name} official requirements`,source_url:sourceUrl,source_checked_at:r.last_verified_at||checkedAt,facts_hash:await hash(evidence),metadata:{requirement_state:r.requirement_state,not_required_inferred:false,completion_clearance:false}},{onConflict:"transaction_id,evidence_key,provider_key,facts_hash",ignoreDuplicates:true});txResult.requirements[key]=r.requirement_state;
