...
function score(r:Row,wanted:string[]){
 ...
 const fs=families(r);for(let i=0;i<wanted.length;i++){if(fs.includes(wanted[i]))s+=120-(i*12)}
 const key=clean(r.provider_key,80);
 if(key==='clerk_land_records')s+=24;if(key==='deeds_mortgages')s+=18;if(key==='liens')s+=18;
 if(/official|public|guest|free/i.test(clean(r.access_mode,100)))s+=6;
 return s;
}
function bestRoute(rows:Row[],wanted:string[]){return [...rows].filter(r=>r.provider_url).sort((a,b)=>score(b,wanted)-score(a,wanted)||label(a,'').localeCompare(label(b,'')))[0]||null}
...
Deno.serve(async(req:Request)=>{
 ...
 for(const tx of txs||[]){
  const ck=countyKey(tx.county),all=(routeMap.get(ck)||[]).filter(r=>r.provider_url);if(!all.length){results.push({transaction_id:tx.id,county:ck,status:"no_registry_route"});continue}
  const lienRoute=bestRoute(all,["liens","clerk_land_records","deeds_mortgages"]),lisRoute=bestRoute(all,["clerk_land_records","legal_notices","liens"]),deedRoute=bestRoute(all,["deeds_mortgages","clerk_land_records"]);
