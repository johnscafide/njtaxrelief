...
Deno.serve(async(req:Request)=>{
  ...
  if(!pins.length)return out(req,200,{records:[],markers:{},meta:{},provider_summary:{available:0,provider_missing:0},plan,source:"Watchdog staging governed fixture provider v2",checked_at:new Date().toISOString()});
  ...
  const markers:Record<string,Record<string,unknown>>={},meta:Record<string,Record<string,unknown>>={},seen=new Set<string>(),now=new Date().toISOString(),currentYear=new Date().getUTCFullYear();
  ...
  for(const r of records||[]){const pin=String(r.pams_pin);for(const id of requested){if(markers[pin]?.[id]!=null)continue;if(id==="watchdog.tax_to_assessment_rate"){const assessed=numeric(r.assessed_value),tax=numeric(r.last_year_tax);if(assessed&&tax!=null){markers[pin][id]=Number((tax/assessed*100).toFixed(6));meta[pin][id]={status:"available",provider_kind:"derived",source:"Watchdog deterministic property formula",observed_at:r.last_seen||now};continue}}if(id==="watchdog.years_since_last_sale"){const year=numeric(r.last_sale_year);if(year!==null&&year>=1800&&year<=currentYear){markers[pin][id]=currentYear-year;meta[pin][id]={status:"available",provider_kind:"derived",source:"Watchdog deterministic sale-recency formula",observed_at:r.last_seen||now};continue}}meta[pin][id]={status:"provider_missing",provider_kind:"none",source:"No staging fixture provider for this marker",checked_at:now}}
  }
  ...
  return out(req,200,{records:records||[],markers,meta,provider_summary:summary,plan,source:"Watchdog staging governed fixture provider v2",provider_versions:{fixture:"staging-public-record-v2"},checked_at:now});
});
