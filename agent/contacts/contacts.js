(function(){
  ...
  var current={meta:null,rawHeaders:[],rawRows:[],mapping:{},contacts:[],selected:new Set(),page:1,search:'',quality:'all',persisted:false,nameMode:'split',namePrompted:false};
  var toastTimer=null;
  ...
  function applyNameMode(mode,opts){
    mode=mode==='together'?'together':'split';current.nameMode=mode;current.mapping._name_mode=mode;
  ...
  function setCurrent(meta,headers,rows,mapping,persisted,promptName){
    current.meta=meta||{};current.rawHeaders=headers||[];current.rawRows=rows||[];current.mapping=mapping&&Object.keys(mapping).length?Object.assign({},mapping):autoMap(current.rawHeaders);current.persisted=!!persisted;current.search='';current.quality='all';current.page=1;current.namePrompted=false;
  ...
  function bind(){
    ...
    var map=q('#acx-map-grid');if(map)map.addEventListener('change',function(e){var sel=e.target.closest('select[data-map-field]');if(!sel)return;var key=sel.getAttribute('data-map-field'),value=sel.value;current.mapping[key]=value;if((key==='first_name'||key==='last_name')&&value&&value.charAt(0)!=='@'){current.mapping[key==='first_name'?'_name_first_source':'_name_last_source']=value;current.mapping._full_name='';current.mapping._name_mode='split';current.nameMode='split';}renderMap();rebuildContacts();saveMappingSoon();});
