...
function renderPacks(){
  ...
  host.className='awf-detail';var s=p.property_snapshot||{},checks=p.checklist||{},qs=encodeURIComponent(p.address),report='/property/report-builder/?pams_pin='+encodeURIComponent(p.pams_pin||'')+'&preset=broker_listing&title='+encodeURIComponent('Listing Prep · '+p.address),tx='/transaction/?prefill_address='+qs+'&prefill_side=seller&prefill_client='+encodeURIComponent(p.client_label||'')+'&prefill_pams_pin='+encodeURIComponent(p.pams_pin||'')+'&prefill_municipality='+encodeURIComponent(p.municipality||s.town||'')+'&prefill_county='+encodeURIComponent(p.county||s.county||'');
  var definitions=[
